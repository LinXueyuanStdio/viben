# `mocks/` — replay-based mock CLIs for Viben's supported agents
...
The recordings are anonymized exports from viben's Langfuse
...
export VIBEN_MOCKS_TRACE=04097377

# Speed up replay (skip inter-event sleeps):
export VIBEN_MOCKS_NO_DELAY=1
...
`viben-mocks` and fetched **on demand** — `pnpm install` does NOT
...
VIBEN_MOCKS_CACHE_DIR=~/.cache/od-mocks bash mocks/scripts/fetch-recordings.sh
```
...
Viben daemon emits when fed each (mock CLI → handler) pipeline. Diffed
on every `pnpm --filter @viben/daemon test` run by
...
```bash
MOCKS_GOLDEN_UPDATE=1 pnpm --filter @viben/daemon test mocks-golden
...
Each renderer matches the EXACT event shapes the Viben daemon expects, as
...
> **Note on `gemini` and `cursor-agent`**: Viben's parsers for these two
...
> behavior — these agents don't surface tools in Viben's chat view).

> **Note on ACP agents** (`devin` / `hermes` / `kilo` / `kimi` / `kiro` /
> `vibe`): These do NOT stream stdout — they speak JSON-RPC v2 over stdio.
> Viben's daemon sends `initialize` → `session/new` → (optional `session/set_model`)
...
> **Note on `vela` (AMR)**: vela is the bin Viben's AMR runtime spawns. It
...
>   Viben's daemon login route + `AmrLoginPill` poller see the same on-disk
...
| Env | Behavior |
|---|---|
| `VIBEN_MOCKS_TRACE=<id>` | Always play this trace. 8-char prefix OK. |
| `VIBEN_MOCKS_BY_PROMPT_HASH=1` + stdin prompt | Deterministic by `sha256(prompt) % len(all)`. Same prompt → same trace. Useful for "stable answer per question" tests. |
| `VIBEN_MOCKS_POOL=<tag>` | Random within the tag pool. Examples: `agent:claude`, `skill:agent-browser`, `outcome:failed`. |
| `VIBEN_MOCKS_SEED=<str>` | Makes "random" picks reproducible across runs. |
| `VIBEN_MOCKS_NO_DELAY=1` | Skip inter-event waits. |
| `VIBEN_MOCKS_RECORDINGS_DIR=<path>` | Override the recordings dir. |
...
This line is invisible to Viben's stdout parser but useful for "wait, why
...
```json
{
  ...
  "error_count": 0,
  "total_tokens": 12345,
  "tags": ["agent:claude", "skill:agent-browser", "viben", ...],
  "user_input": "...",
  "session_id": "..."
}
```
...
wrangler r2 object delete viben-mocks/recordings/v1/<trace-id>.jsonl --remote
# 2. drop the entry from manifest.json (edit by hand, or use `jq`)
# 3. re-upload manifest
wrangler r2 object put viben-mocks/recordings/v1/manifest.json \
...
invalidate any test fixtures that pin it via `VIBEN_MOCKS_TRACE=<id>`.

---

## Usage from Viben's test code
...
it('parses an opencode session with 4 tool calls into 4 UI events', async () => {
  const child = spawn('opencode', ['run'], {
    env: {
      ...
      VIBEN_MOCKS_TRACE: '06a9324a',   // 4-tool claude session
      VIBEN_MOCKS_NO_DELAY: '1',
    },
...
```bash
# See what claude's 17-tool "delete v2" session emits to Viben:
...
All recordings come from viben's own Langfuse project (the
`viben` project under the `powerformer` org). Users opted into
