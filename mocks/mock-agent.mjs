#!/usr/bin/env node
/**
 * mock-agent.mjs — pretends to be one of Viben's supported agent CLIs
...
function parseArgs(argv) {
  ...
  if (process.env.VIBEN_MOCKS_NO_DELAY === '1') opts.noDelay = true;
...
async function main() {
  ...
  // `vela` dispatches by the first positional arg passed by Viben (login /
  ...
  // prompt for hash-mode (use VIBEN_MOCKS_TRACE or _POOL instead).
  ...
  if (!picked) {
    process.stderr.write(
      ...
      '\n' +
      'Or set VIBEN_MOCKS_RECORDINGS_DIR if you stashed them elsewhere.\n',
    );
    process.exit(3);
  }
