...
//
// Priority order:
//   1. VIBEN_MOCKS_TRACE                → fixed trace id (or prefix)
//   2. VIBEN_MOCKS_BY_PROMPT_HASH=1     → hash(prompt) → trace
//   3. VIBEN_MOCKS_POOL=<tag>           → random within tag pool
//   4. (default)                                → random across all
//
// VIBEN_MOCKS_SEED gives reproducible "random" selection.
// VIBEN_MOCKS_RECORDINGS_DIR overrides the default recordings dir
...
export const DEFAULT_RECORDINGS_DIR =
  process.env.VIBEN_MOCKS_RECORDINGS_DIR ||
...
export async function pickRecording({ prompt } = {}) {
  ...
  //    hash selection. A typo in `VIBEN_MOCKS_TRACE` should surface loudly,
  //    not silently produce a different trace and silently poison a test.
  const fixed = process.env.VIBEN_MOCKS_TRACE;
  if (fixed) {
    ...
    throw new Error(
      `VIBEN_MOCKS_TRACE="${fixed}" set but no matching recording in ${dir}. ` +
  ...
  // 2. prompt-hash
  if (process.env.VIBEN_MOCKS_BY_PROMPT_HASH === '1' && prompt) {
  ...
  //    VIBEN_MOCKS_TRACE policy: if the env is set and matches nothing,
  //    refuse to fall through to global random — surface the typo.
  const pool = process.env.VIBEN_MOCKS_POOL;
  if (pool) {
    ...
    if (candidates.length === 0) {
      throw new Error(
        `VIBEN_MOCKS_POOL="${pool}" matched no recordings in ${dir}. ` +
    ...
    const picked = pickRandom(candidates, process.env.VIBEN_MOCKS_SEED);
    if (picked) return { traceId: picked, path: join(dir, `${picked}.jsonl`), method: 'pool', pool };
  }

  // 4. random
  const picked = pickRandom(all, process.env.VIBEN_MOCKS_SEED);
