...
- [ ] **example client 的 plan approval 没有闭环到 `session/elicitation`**
...
- [ ] **example client 多个 pending server request 会错配 resolver**
...
- [ ] **client-side bridge toolCallId 绑定存在并发错配风险**
...
- [ ] **`session/prompt/cancel` / `session/prompt/view` not found 错误码与文档不一致**
...
- [ ] **example client 的 Viben 扩展字段仍使用 camelCase**
...
- [ ] **文档中 `steer_consumed` 与当前实现主路径不一致**
