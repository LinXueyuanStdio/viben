...
| input | string | ✓ | 用户提示词 |
| cwd | string | 否 | 工作目录 |
| session_id | string | 否 | 会话 ID（用于持久化） |
| agent_config_path | string | 否 | 智能体配置文件路径 (e.g., `/path/to/agents/myagent/AGENTS.md`) |
| resume_session | string | 否 | 恢复现有 SDK 会话 |
...
```json
{
  ...
  "tasks": [
    ...
    {
      "id": "task_1709123456790_def456",
      "status": "pending",
      "agent_id": "CLAUDE_CODE",
      "created_at": 1709123456790,
      "position": 1
    }
  ]
}
