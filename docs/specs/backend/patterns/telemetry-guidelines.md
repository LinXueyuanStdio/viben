...
已实现自定义业务 Metrics，存储在 `packages/core/src/telemetry/metrics.ts`。
...
- WebSocket 升级请求（避免与 @fastify/websocket 冲突）
...
Desktop 应用通过 Gateway API 消费 telemetry 数据：

1. **不需要** 在 desktop 中安装 OpenTelemetry 包
2. 使用 `/api/telemetry/*` 端点获取数据
3. 在 `chat-monitor.tsx` 页面展示 trace 可视化
...
- [Gateway 架构](../modules/gateway/index.md)
- [Route Names 定义](../../../packages/core/src/telemetry/route-names.ts)
- [Trace Viewer 实现](../../../packages/core/src/telemetry/trace-viewer.ts)
