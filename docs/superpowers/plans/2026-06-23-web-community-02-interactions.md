...
- Comment list response must use `{ comments, next_cursor }`; each comment includes `author`, `viewer_has_reacted`, one-level `replies`, `replies_count`, `reactions_count`, `created_at`, and `updated_at`.
- Comment detail route must support `PATCH /api/community/comments/{comment_id}` for editing own active comments and `DELETE` with `mode = delete | hide`. Owner can delete own comment; target entity owner and moderators can hide; deleting or hiding a parent comment also hides visible replies and decrements counts by the affected active comment count.
...
Cover `ensureCommunityEntity`, `getCommunitySummary`, `toggleReaction`, `toggleFavorite`, `listFavorites`, `listComments`, `createComment`, `updateComment`, `deleteComment`, `hideComment`, parent delete/hide cascading replies, and comment-entity reaction count sync. Assert the summary response has nested `entity` and `viewer` objects, including `is_authenticated`, `can_comment`, and `can_moderate`.
...
Test anonymous summary read, authenticated reaction toggle with `reaction_type`, favorite toggle, favorites list item summaries, anonymous comments read DTO, comment create, owner edit, owner delete, entity-owner hide, moderator hide, parent delete cascading replies, non-owner edit rejection, and non-owner delete rejection.
...
Assert icon-only buttons have accessible names for like, favorite, share, comment, and more actions. Assert comment form has a label and error live region.
...
Extend the reader route tests to assert `/read/{user_slug}/{page_id}` creates or loads a `published_page` community entity, renders action and comment regions, refreshes viewer state after login-required actions, and passes the correct `community_entity_id` to comments.
