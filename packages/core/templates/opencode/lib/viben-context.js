...
// Debug logging
const DEBUG_LOG = "/tmp/viben-plugin-debug.log"
...
export class VibenContext {
  constructor(directory) {
    ...
    debugLog("context", "VibenContext initialized", { directory })
  }
  ...
  // ============================================================
  // Viben Project Detection
  // ============================================================

  /**
   * Check if this is a Viben-managed project
   */
  isVibenProject() {
    return existsSync(join(this.directory, ".viben"))
  }

  /**
   * Get current task directory from .viben/.current-task
   */
  getCurrentTask() {
    try {
      const currentTaskPath = join(this.directory, ".viben", ".current-task")
  ...
  shouldSkipHook(hookName) {
    // Not a Viben project? Skip.
    if (!this.isVibenProject()) {
      debugLog("context", `shouldSkipHook(${hookName}): skip - not Viben project`)
      return true
    }
