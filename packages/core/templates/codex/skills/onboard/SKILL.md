...
**The Solution**: The `docs/specs/` directory contains project-specific guidelines that should be read before coding starts.
...
**The Solution**: The `/viben:finish-work` command does a final holistic review, re-verifying code against guidelines AFTER writing, catching drift that occurred during development.
...
Real changes often have cross-cutting concerns that need verification.

**WHAT IT ACTUALLY DOES**:
1. Re-reads guidelines and compares written code against them
2. Runs type checker and linter
3. Reviews all changes holistically
4. Checks cross-layer consistency (data flow, code reuse, import paths)
5. Identifies broader impacts
6. Checks if new patterns should be documented
...
**[1/6] /viben:start** - AI needs project context before touching code
**[2/6] viben task create "Fix bug" --slug fix-bug** - Track work for future reference
**[3/6] Investigate and fix the bug** - Actual development work
**[4/6] /viben:finish-work** - Holistic review and verify code against guidelines
**[5/6] Human tests and commits** - Human validates before code enters repo
**[6/6] /viben:record-session** - Persist memory for future sessions
...
**[1/4] /viben:start** - Resume context from previous session
**[2/4] Fix each CR issue** - Address feedback
**[3/4] /viben:finish-work** - Verify fixes and document lessons from CR
**[4/4] Human commits, then /viben:record-session** - Preserve CR lessons
...
**[3/5] Execute phase by phase** - Incremental development
...
**[1/4] /viben:start** - See if this bug was investigated before
**[2/4] Investigation** - Actual debugging work
**[3/4] /viben:finish-work** - Verify changes and document debug findings
**[4/4] Human commits, then /viben:record-session** - Debug knowledge is valuable
...
2. **Read guidelines** - Review `docs/specs/` before coding for project-specific knowledge.
3. **Finish work properly** - /viben:finish-work catches context drift and verifies code.
...
If guidelines are empty templates (contain "To be filled by the team"), this is the first time using Viben in this project.
...
The templates contain placeholder text that needs to be replaced with YOUR project's actual conventions. Without this, AI won't have useful project-specific guidance to follow.
...
"Great! Your team has already customized the development guidelines.
...
"You're now onboarded to the Viben workflow system! Here's what we covered:
