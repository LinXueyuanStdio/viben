...
```
Development Flow:
  Learn something → /viben:update-spec → Knowledge captured
       ↑                                  ↓
  /viben:break-loop ←──────────────────── Future sessions benefit
  (deep bug analysis)
```

- `/viben:break-loop` - Analyzes bugs deeply, often reveals spec updates needed
- `/viben:update-spec` - Actually makes the updates (this command)
- `/viben:finish-work` - Reminds you to check if specs need updates
