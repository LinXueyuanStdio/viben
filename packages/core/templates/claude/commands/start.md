...
```bash
TASK_DIR=$(./.viben/scripts/task.sh create "<title from research>" --slug <suggested-slug>)
```
...
```bash
./.viben/scripts/task.sh init-context "$TASK_DIR" <type>
...
```bash
./.viben/scripts/task.sh start "$TASK_DIR"
```
