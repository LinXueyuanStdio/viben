...
#
# Usage:
#   ./.viben/scripts/create-bootstrap.sh [project-type]
#
# Arguments:
#   project-type: frontend | backend | fullstack (default: fullstack)
#
# Prerequisites:
#   - .viben/.developer must exist (run init-developer.sh first)
#
# Creates:
...
./.viben/scripts/task.sh archive 00-bootstrap-guidelines
```
  ...
  cat > "$dir/task.json" << EOF
{
  ...
  "notes": "First-time setup task created by viben init ($project_type project)"
}
EOF
}
