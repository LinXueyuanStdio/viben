...
class CLIAdapter:
    ...
    def get_agent_name(self, agent: str) -> str:
        ...
        Returns:
            Platform-specific agent name (e.g., 'viben-plan' for OpenCode)
        """
    ...
    def get_commands_path(self, project_root: Path, *parts: str) -> Path:
        ...
        Args:
            project_root: Project root directory
            *parts: Additional path parts (e.g., 'viben', 'finish-work.md')
        ...
        if self.platform == "antigravity":
            ...
            if not parts:
                return workflow_dir
            if len(parts) >= 2 and parts[0] == "viben":
                filename = parts[-1]
        ...
        if self.platform == "cursor" and len(parts) >= 2 and parts[0] == "viben":
            # Convert viben/<name>.md to viben-<name>.md
            filename = parts[-1]
            return self.get_config_dir(project_root) / "commands" / f"viben-{filename}"

        return self.get_config_dir(project_root) / "commands" / Path(*parts)

    def get_viben_command_path(self, name: str) -> str:
        ...
        if self.platform == "cursor":
            return f".cursor/commands/viben-{name}.md"
        elif self.platform == "codex":
            return f".agents/skills/{name}/SKILL.md"
        elif self.platform == "kiro":
            return f".kiro/skills/{name}/SKILL.md"
        elif self.platform == "gemini":
            return f".gemini/commands/viben/{name}.toml"
        elif self.platform == "antigravity":
            return f".agent/workflows/{name}.md"
        else:
            return f"{self.config_dir_name}/commands/viben/{name}.md"
...
def detect_platform(project_root: Path) -> Platform:
    ...
    env_platform = os.environ.get("VIBEN_PLATFORM", "").lower()
