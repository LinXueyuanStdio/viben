...
def _resolve_task_dir(target_dir: str, repo_root: Path) -> Path:
    ...
    if "/" in target_dir or target_dir.startswith(".viben"):
...
def get_check_context(dev_type: str, repo_root: Path) -> list[dict]:
    ...
    entries = [
        {"file": adapter.get_viben_command_path("finish-work"), "reason": "Finish work checklist"},
    ]

    if dev_type in ("backend", "fullstack"):
        entries.append({"file": adapter.get_viben_command_path("check-backend"), "reason": "Backend check spec"})
    if dev_type in ("frontend", "fullstack"):
        entries.append({"file": adapter.get_viben_command_path("check-frontend"), "reason": "Frontend check spec"})
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not full_path.is_dir():
        ...
        print("Hint: Use task name (e.g., 'my-task') or full path (e.g., '.viben/tasks/01-31-my-task')")
        return 1
...
def cmd_set_branch(args: argparse.Namespace) -> int:
    ...
    print(f"  python3 ./.viben/scripts/multi_agent/start.py {args.dir}")
    return 0
...
Examples:
  ...
  python3 task.py init-context .viben/tasks/01-21-add-login backend
  python3 task.py add-context <dir> implement .viben/spec/backend/auth.md "Auth guidelines"
  python3 task.py set-branch <dir> task/add-login
  python3 task.py start .viben/tasks/01-21-add-login
