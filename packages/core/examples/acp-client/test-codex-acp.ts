#!/usr/bin/env npx tsx
/**
 * Codex ACP WebSocket 测试脚本
 * 用于测试 Codex ACP 后端通信流程
 *
 * Usage:
 *   npx tsx test-codex-acp.ts [options]
 *
 * Options:
 *   --url <url>      WebSocket URL (default: ws://127.0.0.1:18790/ws/agent/acp?cwd=/root/viben)
 *   --output <file>  Output file (default: ./codex-acp-dump.jsonl)
...
async function main() {
  console.log("=== Codex ACP WebSocket Test ===");
  ...
  ws.on("message", async (data) => {
    ...
    if (message.id === 1 && message.result) {
      console.log("\n=== Initialized, creating Codex session ===\n");
    ...
    if (message.result && typeof message.result === "object" && "sessionId" in message.result) {
      ...
      console.log("\n=== Codex session created:", sessionId, "===\n");
