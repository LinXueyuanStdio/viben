...
/**
 * Simplified options for running work phase in current repo
 */
export interface WorkPhaseSimpleOptions {
  /** Platform to use (default: "claude") */
  platform?: string;
  /** Enable verbose output */
  verbose?: boolean;
...
/**
 * Run work phase in current repo (convenience wrapper)
 *
 * @param repoRoot - Repository root path
 * @param taskDir - Task directory path (relative or absolute)
 * @param options - Simple options
 * @returns WorkPhaseResult
 */
export async function runWorkPhaseInRepo(
  repoRoot: string,
  taskDir: string,
  options?: WorkPhaseSimpleOptions
): Promise<WorkPhaseResult> {
  // Resolve taskDir to absolute path
