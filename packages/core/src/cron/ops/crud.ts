...
/**
 * Per-path async mutex to serialize concurrent read-modify-write cycles.
 * Prevents file corruption when multiple jobs update status simultaneously.
 */
const configLocks = new Map<string, Promise<void>>();

function withConfigLock<T>(configPath: string, fn: () => Promise<T>): Promise<T> {
  ...
  configLocks.set(configPath, next.then(() => {}, () => {}));
  return next;
}
...
/**
 * Save config to file using atomic write (write to temp, then rename)
 */
...
export async function createJob(
  ...
  // Validate schedule (no lock needed for validation)
