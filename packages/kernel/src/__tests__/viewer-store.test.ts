...
describe('ViewerStore', () => {
  ...
  describe('storeFile() — path outside workspace (issue #195)', () => {
    it('returns null for viben files dir outside workspace without throwing', () => {
      // Regression: voice attachments saved to ~/.viben/files/<sessionId>/ are
      ...
      const id = store.storeFile(
        'sess-1',
        '/home/codex/.viben/files/wAmobthop7go/voice.wav',
        'binary data',
    ...
    it('returns null for storeDiff with viben files dir outside workspace', () => {
  ...
  describe('storeFile()', () => {
    ...
    it('rejects files in sibling directories', () => {
      ...
      const id = store.storeFile('sess-1', '/home/user/projects/other-repo/secret.env', 'code', '/home/user/projects/viben')
      expect(id).toBeNull()
    })
