...
Bidirectional session transfer between Claude CLI and Viben (Telegram). Users can hand off an in-progress Claude CLI session to Telegram and vice versa. Designed agent-agnostic — starts with Claude support, extensible to other agents.
...
- **`UserPromptSubmit` hook stdout is injected as per-message context**: Claude CLI injects hook output into the conversation context for the current prompt, making it readable by the `/viben:handoff` command in the same turn.

## Direction 1: CLI → Telegram (`viben adopt`)

### Flow

1. User chatting in Claude CLI (standalone, not via Viben)
2. User types `/viben:handoff` in Claude CLI
3. Claude CLI hook has injected `session_id` and `cwd` into context
4. Claude reads context, runs: `viben adopt claude <session_id> --cwd <cwd>`
5. CLI command calls `POST /api/sessions/adopt`
6. Core checks if session already exists in Viben:
...
### Adopt Core Logic (`VibenCore.adoptSession()`)
...
```
viben adopt <agent> <session_id> [--cwd <path>]
```

Flow:
1. Read API port from `~/.viben/api.port` (error if daemon not running)
...
| File | Purpose |
|------|---------|
| `~/.claude/hooks/viben-inject-session.sh` | UserPromptSubmit hook — outputs session_id and cwd to context |
| `~/.claude/hooks/viben-handoff.sh` | Script that calls `viben adopt` |
| `~/.claude/commands/viben:handoff.md` | Slash command `/viben:handoff` |
| `~/.claude/settings.json` | Hook registration (merged, not overwritten) |

### Hook: `viben-inject-session.sh`
...
### Handoff Script: `viben-handoff.sh`
    ...
    "UserPromptSubmit": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "~/.claude/hooks/viben-inject-session.sh"
          }
        ]
      }
    ]
...
4. Check if any entry's hooks contain command matching `viben-inject-session.sh` → skip if so
...
3. Create `~/.claude/hooks/viben-inject-session.sh` (chmod +x)
4. Create `~/.claude/hooks/viben-handoff.sh` (chmod +x)
5. Create `~/.claude/commands/viben:handoff.md`
...
1. Delete `~/.claude/hooks/viben-inject-session.sh`
2. Delete `~/.claude/hooks/viben-handoff.sh`
3. Delete `~/.claude/commands/viben:handoff.md`
4. Remove viben hook entry from `~/.claude/settings.json` (filter by command containing `viben-`, keep other hooks)
...
"Install handoff command for Claude CLI? (Y/n)"
  ...
  → n: skip, mention `viben integrate claude` for later
```
...
| `VibenCore` | `adoptSession(agent, agentSessionId, cwd)` | Full adopt flow |
...
| `README.md` | Add Session Handoff section: feature overview, `viben adopt` usage, `viben integrate claude` usage, `/handoff` Telegram command, flow diagrams for both directions |
