...
initLogger() ─→ pino root ──┤
                            └─── pino-roll ────→ ~/.viben/logs/viben.log (JSON lines)
```

Each session additionally creates a file transport writing to `~/.viben/logs/sessions/<sessionId>.log`.
...
- **Path:** `~/.viben/logs/sessions/<sessionId>.log`
...
Added to `~/.viben/config.json` under a `logging` key:
...
|---------|-----------|---------|
| `VIBEN_LOG_LEVEL` | `logging.level` | `debug`, `info`, `warn`, `error` |
| `VIBEN_LOG_DIR` | `logging.logDir` | `/var/log/viben` |
| `VIBEN_DEBUG` | Sets level to `debug` | `1` or `true` |

**Priority:** `VIBEN_LOG_LEVEL` > `VIBEN_DEBUG` > config file > defaults

If both `VIBEN_LOG_LEVEL=warn` and `VIBEN_DEBUG=1` are set, `VIBEN_LOG_LEVEL` wins (more specific).
...
The Zod enum includes `'silent'` for use in tests: `VIBEN_LOG_LEVEL=silent` suppresses all output.
...
- Reads `~/.viben/logs/sessions/`, checks file mtime
