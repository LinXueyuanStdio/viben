...
Two features for Viben:
1. **`viben config`** — Menu-based interactive config editor
...
- `runMode: 'daemon'` — `viben` spawns a background process and exits
- `autoStart: true` — installs OS-level auto-start (LaunchAgent on macOS, systemd on Linux)
- Env override: `VIBEN_RUN_MODE` (must be added to `applyEnvOverrides` in config.ts)

## CLI Commands

| Command | Description |
|---|---|
| `viben` (no args) | Reads `runMode` from config. `foreground` → runs server directly. `daemon` → spawns detached child and exits. |
| `viben start` | Force daemon mode (regardless of config) |
| `viben stop` | Reads PID from `~/.viben/viben.pid`, sends SIGTERM |
| `viben status` | Checks PID file, reports running/stopped |
| `viben logs` | Reads `logDir` from config, tails `viben.log` in that dir with `tail -f -n 50` |
| `viben config` | Menu-based config editor |
| `viben --foreground` | Force foreground (overrides `runMode: daemon`) |
| `viben --daemon-child` | Internal flag — actual server process spawned by daemon mode |

### Daemon Spawn Logic

1. Check `~/.viben/viben.pid` — if PID file exists but process is not alive, remove stale PID file. If process alive, print "Already running (PID xxx)" and exit
2. Spawn `node <cli-path> --daemon-child` with `{ detached: true, stdio: 'ignore' }`
3. **Parent** writes child PID to `~/.viben/viben.pid` (single writer — child does NOT write PID)
...
- `viben --foreground` does NOT write a PID file — it runs like the current behavior
- `viben start` and daemon-mode `viben` always check/write PID file
- `viben stop` only affects daemon processes (PID file). Foreground processes are stopped with Ctrl+C as usual
- If a foreground instance is running and user runs `viben start`, the daemon starts independently (no conflict — they are separate processes)
...
File: `~/Library/LaunchAgents/com.viben.daemon.plist`

```xml
<plist version="1.0">
<dict>
  <key>Label</key>
  <string>com.viben.daemon</string>
  <key>ProgramArguments</key>
  <array>
    <string>/path/to/node</string>
    <string>/path/to/viben/cli.js</string>
  ...
  <string>/Users/username/.viben/logs/viben.log</string>
...
File: `~/.config/systemd/user/viben.service`

```ini
[Unit]
Description=Viben Daemon

[Service]
ExecStart=/path/to/node /path/to/viben/cli.js --daemon-child
Restart=on-failure
...
- Install: `systemctl --user daemon-reload && systemctl --user enable viben`
- Uninstall: `systemctl --user disable viben` + delete file
...
[3/3] Run Mode
  ...
  ❯ Background (daemon)
    ...
    Manage with: viben status | stop | logs
...
- **Background** → `runMode: 'daemon'`, `autoStart: true` → install OS service → spawn daemon → print "Viben is running in background (PID xxx)"
