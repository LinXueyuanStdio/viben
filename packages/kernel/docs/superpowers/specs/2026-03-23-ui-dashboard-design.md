# Viben UI Dashboard — Design Specification

## Overview

Embedded web dashboard for monitoring and managing Viben instances. Served directly from the existing API server as static files, providing session management, system health monitoring, agent overview, configuration editing, and topic management.

## Architecture

### Deployment Model

Embedded SPA served by Viben's existing `http.createServer` API server on the same port (default 21420). No separate process or port required.
...
2. **`VibenCore`** creates and owns the `EventBus` instance
