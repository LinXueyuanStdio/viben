...
- **Single CLI entry point**: `viben config` is the primary config command. Smart-routes to API when server is running, file when not. Deprecate `viben api config` (keep working with warning).
...
- Assistant asks user → receives value → runs `viben config set security.maxConcurrentSessions <value>` → confirms result
...
### Deprecate `viben api config`

Keep `viben api config` and `viben api config set` working but print deprecation warning: `"Deprecated: use 'viben config' or 'viben config set' instead."` This follows the project's backward compatibility policy (never remove existing commands, deprecate with warning).

### `viben config` (interactive)

1. Detect server running (read `~/.viben/api.port` + health check)
...
### `viben config set <key> <value>` (non-interactive, for scripting)

Same smart routing: server running → API, not running → file. Primary replacement for `viben api config set`.

### `cmdConfig()` argument handling

Current `cmdConfig()` takes no arguments. Updated to parse `args`:
- `viben config` (no args) → interactive editor
- `viben config set <key> <value>` → non-interactive set
...
- Replace `viben api config set` → `viben config set` in `buildAssistantSystemPrompt()`. Only change config-specific references — other `viben api` commands (`api new`, `api status`, etc.) remain unchanged as they route through the running daemon.
- Add instruction: when user asks about "settings" or "config", assistant runs `viben config set` directly
- Add instruction: when receiving a delegated config change from Settings menu, ask user for value and apply

### Delegation flow

Settings menu → sends message to assistant topic → assistant asks user for value → runs `viben config set <key> <value>` → confirms result. No new tools or APIs needed — uses existing shell command execution.
