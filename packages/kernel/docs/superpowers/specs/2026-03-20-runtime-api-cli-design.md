...
Add an HTTP control API to the Viben daemon and a `viben api` CLI subcommand to interact with it. This allows users to create sessions, cancel sessions, check status, and list agents from the terminal without using Telegram.

## Context

Viben currently manages sessions exclusively through the Telegram adapter. Bot commands like `/new`, `/cancel`, `/status`, `/agents` are only available in Telegram. This spec adds a parallel control path via HTTP API + CLI.

## Architecture

```
CLI (viben api new)
  → reads ~/.viben/api.port
...
Environment variable override: Add `['VIBEN_API_PORT', ['api', 'port']]` to the `applyEnvOverrides` array with numeric cast (same pattern as `chatId`).
...
On start, the server writes the actual listening port to `~/.viben/api.port`. On stop, it removes this file. This allows the CLI to discover the port even if it differs from config (e.g., fallback on port conflict).
...
export class ApiServer {
  constructor(private core: VibenCore, private config: ApiConfig) {}

  async start(): Promise<void>
  // Creates http.Server, listens on config.host:config.port
  // Writes port to ~/.viben/api.port
  // Logs "API server listening on {host}:{port}"

  async stop(): Promise<void>
  // Closes http.Server
  // Removes ~/.viben/api.port file
}
```
...
```
viben api new [agent] [--workspace path]   Create a new session
viben api cancel <session-id>               Cancel a session
...
Each api command:
1. Reads `~/.viben/api.port` to get the port
2. If file missing: prints "Viben is not running. Start with `viben start`" and exits with code 1
3. Uses native `fetch()` (Node 18+) to call `http://127.0.0.1:{port}/api/...`
4. If connection refused: prints "Viben is not running (stale port file)", **removes the stale `api.port` file**, and exits with code 1
...
- **Unit tests** for API server: mock `VibenCore`, test each endpoint returns correct JSON and status codes
