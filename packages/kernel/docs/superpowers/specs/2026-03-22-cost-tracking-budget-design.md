...
- CLI `viben usage` command (Telegram only for now)
...
usage event (ACP)
    │
    ▼
VibenCore.wireSessionEvents()
    │
    ├──► UsageStore.append(record)     ── ~/.viben/usage.json
    │
    └──► UsageBudget.check()
              │
...
JSON file store at `~/.viben/usage.json`. Follows the same pattern as `JsonFileSessionStore`.
...
### Initialization inside `VibenCore` constructor

Following the existing pattern where `VibenCore` creates its own `JsonFileSessionStore`, `SessionManager`, and `NotificationManager` internally (see `core.ts` lines 31–41), the `UsageStore` and `UsageBudget` are also created inside the constructor:

```typescript
// In VibenCore constructor (core.ts)
...
Properties exposed on `VibenCore` so Telegram adapter can access for `/usage` command:
```typescript
...
Register in `commands.ts` alongside existing commands. The handler accesses usage data via `core.usageStore` (exposed as public property on `VibenCore`). Reuse existing `progressBar()` and `formatTokens()` helpers from `formatting.ts` for consistent visual style.
