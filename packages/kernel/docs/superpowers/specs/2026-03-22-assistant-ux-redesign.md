...
Redesign the Telegram Assistant experience to be smarter, more user-friendly, and capable of guiding users through all Viben features. The assistant should be approachable for new users while remaining powerful for experienced ones.
...
- **Assistant behavior**: Self-execute via `viben api ...` when possible, guide user for Telegram-only actions.
...
The current `/new` command is confusing — it silently uses `baseDir` as workspace, but `baseDir` is just the **base directory** (e.g. `~/viben-workspace`), not a specific project folder. Users don't understand where the agent will work.

#### Key concept

**Workspace = the project directory the agent works in.** Not the base dir. For example: `~/viben-workspace/my-app` or `~/code/my-project`. The agent reads, writes, and executes code inside this directory.
...
5. Create via `viben api new <agent> <workspace>`
...
[📁 Default (~/viben-workspace)]  [✏️ Custom path]
```
...
Tell the assistant what Viben is and who uses it:

```
You are the Viben Assistant — a helpful guide for managing AI coding sessions.

Viben bridges messaging platforms (like Telegram) to AI coding agents (like Claude Code)
...
- Run `viben api new <agent> <workspace>` to create the session
...
- Self-execute: `viben api status`, `viben api topics`
- Respond with formatted data

**Cancel session:**
- Self-execute: `viben api status` to see active sessions
- If 1 active → ask confirm → `viben api cancel <id>`
- If multiple → list, ask user to pick

**Troubleshoot (session stuck, errors):**
- Self-execute: `viben api health` + `viben api status`
- Diagnose the problem
- Small issue (stuck session) → suggest cancel + create new
- Big issue (system problem) → suggest restart, ask confirm

**Cleanup:**
- Self-execute: `viben api topics --status finished,error`
- Report count, ask confirm
- Execute: `viben api cleanup --status <statuses>`

**Config:**
- Self-execute: `viben api config` (view), `viben api config set <key> <value>` (update)
...
- Self-execute: `viben api dangerous <id> on|off`
- Explain what it does if user seems unsure

#### 4.4 Guidelines

- Self-execute `viben api ...` for everything you can. Only guide user to Telegram actions when needed (creating sessions, typing in topics).
...
Keep the current CLI command reference (`viben api ...`) for the assistant to use. Remove the separate "Session Management Commands" section (Telegram bot commands like `/new`, `/cancel`) from the prompt — the playbook entries already contain the relevant Telegram commands where needed (e.g., "type `/new <agent> <workspace>`" in the Create Session playbook entry).

**Note on self-execution:** The assistant can run `viben api ...` commands because the adapter already auto-approves permission requests containing "viben" (existing behavior in adapter.ts). This spec does not change that behavior.
