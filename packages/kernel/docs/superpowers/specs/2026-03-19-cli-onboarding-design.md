...
Interactive first-run setup for Viben. When a user runs `viben` for the first time (no config file exists), the app enters an interactive wizard that walks them through configuring Telegram, agents, workspace, and security — then optionally starts Viben immediately.
...
viben (first run, no config)
  ...
  │    │   (auto-created by Viben on first start)
  ...
  │    └─ Input: base directory (default: ~/viben-workspace)
  │
  ...
  └─ Step 6: Save & Start
       ...
       └─ Ask "Start Viben now?" → Yes: start / No: exit with instructions
```
...
- `getConfigPath(): string` — returns the resolved config path (respects `VIBEN_CONFIG_PATH` env var)
...
For now, users delete `~/.viben/config.json` and run `viben` again. In Phase 2, a dedicated `viben setup` command can be added with `--force` flag.
