...
Add a built-in Discord adapter to Viben with full feature parity with the Telegram adapter. Uses discord.js library and maps Telegram concepts (forum topics, inline keyboards, callback queries) to Discord equivalents (forum channel posts, button components, interactions).
...
Auto-approve: `session.dangerousMode = true` or description contains `"viben"`.
...
- Add `VIBEN_DISCORD_BOT_TOKEN` and `VIBEN_DISCORD_GUILD_ID` to `applyEnvOverrides()` array
