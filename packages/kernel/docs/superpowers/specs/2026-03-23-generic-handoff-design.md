...
Bi-directional session handoff between terminal agents and Viben (Telegram) for ALL supported agents. Replaces the Claude-only implementation with a data-driven integration engine.
...
- Manual `viben adopt` for agents without hooks system
- Resume capabilities unified (by ID and resume latest treated the same)

## Agent Capability Matrix

Based on research of all 28 agents in the Viben registry:
...
interface AgentIntegrationSpec {
  ...
  handoffCommandName?: string;          // "viben:handoff" or "viben-handoff"
...
- `"markdown"`: Single `.md` file in commands dir (Claude) — e.g., `~/.claude/commands/viben:handoff.md`
- `"skill"`: Subdirectory with `SKILL.md` (Cursor) — e.g., `~/.cursor/skills/viben-handoff/SKILL.md`
...
const AGENT_CAPABILITIES: Record<string, AgentCapability> = {
  claude: {
    ...
    integration: {
      ...
      handoffCommandName: "viben:handoff",
...
function generateHandoffScript(agentKey: string): string {
  return `#!/bin/bash
if [ -z "$1" ]; then
  echo "Usage: viben-handoff.sh <session_id> [cwd]"
  exit 1
fi
viben adopt ${agentKey} "$1" \${2:+--cwd "$2"}`;
}
```
...
async function installIntegration(agentKey: string, spec: AgentIntegrationSpec): Promise<IntegrationResult> {
  ...
  const injectPath = path.join(spec.hooksDirPath, "viben-inject-session.sh");
  await writeScript(injectPath, generateInjectScript(agentKey, spec));

  // 3. Write handoff script + chmod 755
  const handoffPath = path.join(spec.hooksDirPath, "viben-handoff.sh");
  ...
  if (spec.commandsPath && spec.handoffCommandName) {
    if (spec.commandFormat === "skill") {
      // Cursor style: ~/.cursor/skills/viben-handoff/SKILL.md
      ...
      // Claude style: ~/.claude/commands/viben:handoff.md
...
async function uninstallIntegration(agentKey: string, spec: AgentIntegrationSpec): Promise<IntegrationResult> {
  // Remove hook files with "viben-" prefix from hooksDirPath
...
async function mergeAgentSettings(
  ...
  // 2. Backup to .bak
  // 3. Check if entry with "viben-inject-session.sh" already exists → skip
...
3. User triggers handoff:
   - Claude: `/viben:handoff` slash command (auto-runs adopt)
   - Cursor: `/viben-handoff` skill (auto-runs adopt)
   - Gemini/Cline: user manually runs `viben adopt <agent> <session_id> --cwd <path>`
4. `POST /api/sessions/adopt` → `core.adoptSession()` → resume on Telegram

**Agents without hooks (Codex, Kilo, Amp):**
1. User manually runs: `viben adopt codex <session_id> --cwd <path>`
2. Same API flow
...
**Limitation note**: Cline (`cline --continue`) and Kilo (`kilo --continue`) resume the most recent session, not a specific session by ID. The `/handoff` command will show this generic resume command, which may not resume the exact Viben session if the user has other sessions open.
...
- Users who ran `viben integrate claude` already have files installed
- `installIntegration()` detects existing files via marker (`viben-inject-session.sh`)
...
- `viben integrate <agent>` CLI command continues to work, delegates to same engine
- `viben integrate <agent>` is idempotent — re-running updates files without duplicating
