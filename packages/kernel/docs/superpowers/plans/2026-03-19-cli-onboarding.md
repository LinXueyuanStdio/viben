...
Run:
```bash
cd . && pnpm add -D vitest --filter @viben/core
```
...
Run:
```bash
cd . && pnpm --filter @viben/core test
```
...
describe('ConfigManager new methods', () => {
  ...
  const originalEnv = process.env.VIBEN_CONFIG_PATH
  ...
  afterEach(() => {
    ...
    if (originalEnv === undefined) {
      ...
      process.env.VIBEN_CONFIG_PATH = originalEnv
    }
  })
  ...
  describe('getConfigPath()', () => {
    ...
    it('respects VIBEN_CONFIG_PATH env var', () => {
      const customPath = path.join(tmpDir, 'custom.json')
      process.env.VIBEN_CONFIG_PATH = customPath
      const cm = new ConfigManager()
  ...
  describe('writeNew()', () => {
    ...
    it('creates parent directory if it does not exist', async () => {
      ...
      process.env.VIBEN_CONFIG_PATH = nestedPath
      const cm = new ConfigManager()
      await cm.writeNew({
...
Run:
```bash
cd . && pnpm add @inquirer/prompts --filter @viben/core
```
...
Run:
```bash
cd . && pnpm --filter @viben/core build
```
...
export async function runSetup(configManager: ConfigManager): Promise<boolean> {
  ...
  try {
    ...
    const shouldStart = await confirm({ message: 'Start Viben now?', default: true })
    return shouldStart
  } catch (err) {
...
describe('runSetup integration', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'viben-setup-'))
    configPath = path.join(tmpDir, 'config.json')
    process.env.VIBEN_CONFIG_PATH = configPath
...
Run:
```bash
[ -f ~/.viben/config.json ] && mv ~/.viben/config.json ~/.viben/config.json.bak
```

- [ ] **Step 3: Run viben to trigger setup wizard**
...
- Config is saved to `~/.viben/config.json`
- "Start Viben now?" prompt works

- [ ] **Step 5: Restore original config**

Run:
```bash
[ -f ~/.viben/config.json.bak ] && mv ~/.viben/config.json.bak ~/.viben/config.json
```
