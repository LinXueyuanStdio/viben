...
**Goal:** Enable bidirectional session transfer between Claude CLI and Viben (Telegram) — adopt external CLI sessions into Telegram, and hand off Telegram sessions back to CLI.

**Architecture:** An `AgentRegistry` module defines per-agent capabilities (resume support, CLI resume commands). `VibenCore.adoptSession()` handles the adopt flow: validates agent, checks store for existing session, spawns+resumes agent, creates Telegram topic, persists to store. `POST /api/sessions/adopt` exposes this via HTTP. CLI commands `viben adopt` and `viben integrate` provide the user interface. Claude CLI integration is installed via hook files + custom command.
...
### Task 3: Implement `adoptSession()` in VibenCore
...
- [ ] **Step 2: Add `adoptSession()` to `VibenCore`**
...
git commit -m "feat: add adoptSession() to VibenCore"
```
...
### Task 5: Add `viben adopt` CLI command
...
export async function cmdAdopt(args: string[]): Promise<void> {
  ...
  if (!agent || !sessionId) {
    console.log("Usage: viben adopt <agent> <session_id> [--cwd <path>]");
...
```
  adopt <agent> <id>  Adopt an external agent session into Viben
...
git commit -m "feat: add viben adopt CLI command"
```
...
### Task 7: Create Claude CLI Integration (`viben integrate claude`)
...
const INJECT_HOOK_FILE = join(HOOKS_DIR, "viben-inject-session.sh");
const HANDOFF_SCRIPT_FILE = join(HOOKS_DIR, "viben-handoff.sh");
const HANDOFF_COMMAND_FILE = join(COMMANDS_DIR, "viben:handoff.md");
...
if [ -z "$SESSION_ID" ]; then
  echo "Usage: viben-handoff.sh <target> <session_id> [cwd]"
  exit 1
fi

viben adopt "$TARGET" "$SESSION_ID" \${CWD:+--cwd "$CWD"}
`;

const HANDOFF_COMMAND_CONTENT = `---
description: Hand off current session to Viben (Telegram)
---

Look at the context injected at the start of this message to find
CLAUDE_SESSION_ID and CLAUDE_WORKING_DIR, then run:

bash ~/.claude/hooks/viben-handoff.sh viben <CLAUDE_SESSION_ID> <CLAUDE_WORKING_DIR>
`;

const HOOK_MARKER = "viben-inject-session.sh";
...
export class ClaudeIntegration implements AgentIntegration {
  ...
  private removeFromSettings(): void {
    ...
    hooks.UserPromptSubmit = (hooks.UserPromptSubmit as Array<{ hooks?: Array<{ command?: string }> }>).filter(
      (group) => !group.hooks?.some((h) => h.command?.includes("viben-")),
    );
...
export async function cmdIntegrate(args: string[]): Promise<void> {
  ...
  if (!agent) {
    console.log("Usage: viben integrate <agent> [--uninstall]");
  ...
  try {
    if (uninstall) {
      ...
      console.log(`  Use /viben:handoff in Claude CLI to hand off sessions.`);
    }
  } catch (err) {
...
git commit -m "feat: add viben integrate claude command"
```
...
if (agents["claude"]) {
  ...
  const installClaude = await confirm({
    message: "Install handoff command for Claude CLI? (enables /viben:handoff in Claude CLI)",
    default: true,
  });
  ...
  if (installClaude) {
    try {
      ...
      console.log("  You can install it later with: viben integrate claude\n");
    }
  }
}
```
...
# In Claude CLI, type /viben:handoff to transfer current session
# Or manually:
viben adopt claude <session_id> --cwd /path/to/project
```
...
Verify with Viben running:
1. `viben adopt --help` shows usage
2. `viben integrate --help` shows available integrations
3. `viben integrate claude` creates files in `~/.claude/`
4. `viben integrate claude --uninstall` removes files cleanly
5. `viben adopt claude fake-session-id` returns appropriate error
