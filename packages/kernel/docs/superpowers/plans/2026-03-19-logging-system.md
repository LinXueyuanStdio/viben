...
```bash
cd /Users/lucas/code/Viben && pnpm add pino pino-roll pino-pretty
```
...
describe('logging config schema', () => {
  ...
  it('allows partial logging overrides', () => {
    ...
    expect(result.logging.logDir).toBe('~/.viben/logs')
  })
...
Note: `VIBEN_DEBUG` only sets debug if `VIBEN_LOG_LEVEL` is not set (priority rule).
...
describe('logger', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'viben-log-test-'))
  })
  ...
  it('initLogger creates log directory and file', async () => {
    ...
    expect(files.some(f => f.startsWith('viben'))).toBe(true)
  })
  ...
  it('createChildLogger adds module context', async () => {
    ...
    const logFile = fs.readdirSync(logDir).find(f => f.startsWith('viben'))
  ...
  const transports = pino.transport({
    targets: [
      ...
      },
      {
        target: 'pino-roll',
        options: {
          file: path.join(resolvedLogDir, 'viben.log'),
...
describe('session logger', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'viben-session-log-'))
  })
  ...
  it('also writes to combined log', async () => {
    ...
    const combinedFile = fs.readdirSync(logDir).find(f => f.startsWith('viben'))
    if (combinedFile) {
...
```typescript
// Before: log.info('Starting Viben server...')
// After:
log.info('Starting Viben server')
...
describe('logger integration', () => {
  ...
  it('full lifecycle: init → child → session → cleanup → shutdown', async () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'viben-int-'))
    const logDir = path.join(tmpDir, 'logs')
  ...
  it('gracefully degrades if log dir is not writable', () => {
    ...
    expect(() => {
      initLogger({ level: 'info', logDir: '/dev/null/viben-test-log', maxFileSize: '10m', maxFiles: 7, sessionLogRetentionDays: 30 })
    }).not.toThrow()
  })
})
```
...
```bash
VIBEN_LOG_LEVEL=debug node dist/main.js
```

Expected: Pretty-printed logs in terminal, log files created in `~/.viben/logs/`. Stop with Ctrl+C and verify files exist.
