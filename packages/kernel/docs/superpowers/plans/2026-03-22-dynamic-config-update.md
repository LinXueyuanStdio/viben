...
**Architecture:** A central config registry (`config-registry.ts`) defines metadata for each config field (scope, type, hot-reload). The API layer uses this registry to determine restart needs and emit change events. The Telegram adapter reads the registry to auto-generate a Settings menu. The CLI `viben config` smart-routes to API when server is running.
...
| `src/adapters/telegram/assistant.ts` | Modify | Update system prompt — `viben config set` instead of `viben api config set` |
...
describe('ConfigManager events', () => {
  ...
  beforeEach(async () => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'viben-config-test-'))
...
Add at the end of the `VibenCore` constructor in `src/core/core.ts:31-43`:
...
```ts
  constructor(configManager: ConfigManager) {
    ...
    const storePath = path.join(os.homedir(), ".viben", "sessions.json");
...
```ts
    configManager: {
      get: vi.fn(() => ({
        ...
        logging: { level: 'info', logDir: '~/.viben/logs', maxFileSize: '10m', maxFiles: 7, sessionLogRetentionDays: 30 },
...
import type { VibenCore } from "../../../core/index.js";
...
function buildSettingsKeyboard(core: VibenCore): InlineKeyboard {
...
export async function handleSettings(ctx: Context, core: VibenCore): Promise<void> {
...
export function setupSettingsCallbacks(
  ...
  bot.callbackQuery(/^s:input:/, async (ctx) => {
    ...
    const prompt = `User wants to change ${fieldDef.displayName} (config path: ${fieldPath}). Current value: ${JSON.stringify(currentValue)}. Ask them for the new value and apply it using: viben config set ${fieldPath} <value>`;
  ...
  bot.callbackQuery("s:back", async (ctx) => {
    ...
    try {
      await ctx.editMessageText(`<b>Viben Menu</b>\nChoose an action:`, {
        parse_mode: "HTML",
...
export async function cmdConfig(args: string[] = []): Promise<void> {
  ...
  if (subCmd === 'set') {
    // Non-interactive: viben config set <key> <value>
...
In `src/cli/commands.ts:15`, add after `viben config`:
```
  viben config set <key> <value>       Set a config value
```

In the API section (line 41-42), add deprecation note:
```
  viben api config                       Show runtime config (deprecated → viben config)
  viben api config set <key> <value>     Update config value (deprecated → viben config set)
```
...
git commit -m "feat(cli): smart-routing viben config with api mode and deprecation"
```
...
```ts
### Configuration
- View: \`viben config\` (or \`viben api config\` — deprecated)
- Update: \`viben config set <key> <value>\`
- When user asks about "settings" or "config", use \`viben config set\` directly
- When receiving a delegated request from the Settings menu, ask user for the new value, then apply with \`viben config set <path> <value>\`
```

Also update the CLI Commands Reference section (lines 176-177):

```ts
viben config                           # Edit config (interactive)
viben config set <key> <value>         # Update config value
viben api config                       # Show config (deprecated)
viben api config set <key> <value>     # Update config (deprecated)
```
...
1. Start Viben: `pnpm build && node dist/cli.js`
...
7. In terminal: `viben config set security.maxConcurrentSessions 30` → verify it routes through API
8. In terminal: `viben api config` → verify deprecation warning shown
