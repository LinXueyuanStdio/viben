...
export async function deleteSessionThread(
  ...
  try {
    const thread = guild.channels.cache.get(threadId) as ThreadChannel | undefined
      ?? await guild.channels.fetch(threadId) as ThreadChannel | null
    if (thread) {
      await thread.delete('Viben session cleanup')
    }
  } catch (err) {
...
export const SLASH_COMMANDS = [
  ...
  new SlashCommandBuilder().setName('restart').setDescription('Restart Viben'),
...
export async function handleRestart(
  ...
  await interaction.reply({ content: 'Restarting Viben...', ephemeral: true })
...
export async function handleMenu(
  ...
  await interaction.reply({ content: '**Viben Menu**', components: buildMenuKeyboard(), ephemeral: true })
}
...
export function buildWelcomeMessage(core: VibenCore): string {
  const sessions = core.sessionManager.listSessions()
  const active = sessions.filter(s => s.status === 'active')

  if (active.length === 0) {
    return '👋 **Viben Assistant** ready. Use `/new` to create a session or ask me anything!'
  }

  const lines = active.map(s => `• **${s.name ?? s.id}** (${s.agentName}) — ${s.status}`)
  return `👋 **Viben Assistant** ready.\n\n**Active sessions:**\n${lines.join('\n')}\n\nUse `/new` to create a session or ask me anything!`
}

function buildAssistantSystemPrompt(core: VibenCore): string {
  const config = core.configManager.get()
  const sessions = core.sessionManager.listSessions()
  ...
  return `You are the Viben Assistant running in Discord. You help users manage their AI coding agent sessions.
...
export class DiscordAdapter extends ChannelAdapter<VibenCore> {
  ...
  constructor(core: VibenCore, config: DiscordChannelConfig) {
    super(core, config)
  ...
  private async setupAssistant(): Promise<void> {
    if (!this.discordConfig.assistantThreadId) {
      ...
      const thread = await this.forumChannel.threads.create({
        name: '🤖 Assistant',
        message: { content: 'Viben Assistant thread' },
        reason: 'Viben assistant',
      })
