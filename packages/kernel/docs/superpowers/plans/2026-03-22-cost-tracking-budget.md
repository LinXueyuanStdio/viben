...
describe("UsageStore", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "viben-usage-test-"));
...
describe("UsageBudget", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "viben-budget-test-"));
...
Add properties to the `VibenCore` class, after `private resumeLocks`:
...
git commit -m "feat(core): wire UsageStore and UsageBudget into VibenCore event loop"
```
...
```typescript
async function handleUsage(ctx: Context, core: VibenCore): Promise<void> {
  if (!core.usageStore) {
