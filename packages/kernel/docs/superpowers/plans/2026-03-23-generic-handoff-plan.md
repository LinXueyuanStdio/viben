...
viben adopt ${agentKey} "$SESSION_ID" \${CWD:+--cwd "$CWD"}
`;
}
...
function generateHandoffCommand(agentKey: string, spec: AgentIntegrationSpec): string {
  ...
  return `---
description: Transfer current session to Viben (Telegram)
---

Look at the context injected at the start of this message to find
${sidVar} and ${cwdVar}, then run:

bash ${expandPath(spec.hooksDirPath)}viben-handoff.sh <${sidVar}> <${cwdVar}>
`;
}
...
function buildIntegrationItem(agentKey: string, spec: AgentIntegrationSpec): IntegrationItem {
  ...
  return {
    ...
    isInstalled(): boolean {
      return existsSync(join(hooksDir, "viben-inject-session.sh")) &&
...
Expected: Creates `~/.gemini/hooks/viben-inject-session.sh` and `viben-handoff.sh`, merges `~/.gemini/settings.json`
