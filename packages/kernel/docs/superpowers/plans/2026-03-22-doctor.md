# Viben Doctor Implementation Plan
...
export class DoctorEngine {
  ...
  private async buildContext(): Promise<DoctorContext> {
    const dataDir = path.join(os.homedir(), ".viben");
...
const BIN_DIR = path.join(os.homedir(), ".viben", "bin");
...
In `src/cli/commands.ts` `printHelp()`, add after the `viben update` line:
```
  viben doctor                         Run system diagnostics
