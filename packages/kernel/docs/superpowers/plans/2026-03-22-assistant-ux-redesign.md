...
**Goal:** Redesign the Telegram Assistant UX to be context-aware, user-friendly, and capable of guiding users through all Viben features.
...
describe("buildWelcomeMessage", () => {
  it("shows no-sessions variant when totalCount is 0", () => {
    ...
    expect(msg).toContain("Viben is ready");
...
function makeCtx(overrides?: Partial<AssistantContext>): AssistantContext {
  return {
    config: {
      ...
      defaultAgent: "claude",
      workspace: { baseDir: "~/viben-workspace" },
    } as any,
...
describe("buildAssistantSystemPrompt", () => {
  it("includes product context", () => {
    ...
    expect(prompt).toContain("Viben Assistant");
  ...
  it("includes action playbook", () => {
    ...
    expect(prompt).toContain("viben api cancel");
    expect(prompt).toContain("viben api health");
    expect(prompt).toContain("viben api cleanup");
    expect(prompt).toContain("viben api config");
  });

  it("includes guidelines about self-execution", () => {
    const prompt = buildAssistantSystemPrompt(makeCtx());
    expect(prompt).toContain("viben api");
...
export function buildAssistantSystemPrompt(ctx: AssistantContext): string {
  ...
  return `You are the Viben Assistant — a helpful guide for managing AI coding sessions.

Viben bridges messaging platforms (like Telegram) to AI coding agents (like Claude Code) via the Agent Client Protocol (ACP). Users chat here, and their messages are routed to AI agents that can read, write, and execute code in real workspaces.
...
- Create via: \`viben api new <agent> <workspace>\`

### Check Status / List Sessions
- Run \`viben api status\` for active sessions overview
- Run \`viben api topics\` for full list with statuses
- Format the output nicely for the user

### Cancel Session
- Run \`viben api status\` to see what's active
- If 1 active session → ask user to confirm → \`viben api cancel <id>\`
- If multiple → list them, ask user which one to cancel

### Troubleshoot (Session Stuck, Errors)
- Run \`viben api health\` + \`viben api status\` to diagnose
- Small issue (stuck session) → suggest cancel + create new
- Big issue (system-level) → suggest restart, ask for confirmation first

### Cleanup Old Sessions
- Run \`viben api topics --status finished,error\` to see what can be cleaned
- Report the count, ask user to confirm
- Execute: \`viben api cleanup --status <statuses>\`

### Configuration
- View: \`viben api config\`
- Update: \`viben api config set <key> <value>\`
...
- Run \`viben api dangerous <id> on|off\`
...
viben api new <agent> <workspace>      # Create new session
...
# Topic management
viben api topics                       # List all topics
...
## Guidelines
- Run \`viben api ...\` commands yourself for everything you can. Only guide users to Telegram buttons/commands when needed (e.g., creating sessions requires a Telegram topic).
