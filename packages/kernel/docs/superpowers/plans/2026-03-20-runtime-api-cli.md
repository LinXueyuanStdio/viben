...
**Goal:** Add an HTTP control API to the Viben daemon and `viben api` CLI commands to create/cancel sessions, list sessions, and list agents.

**Architecture:** A lightweight HTTP server (`ApiServer`) using Node's native `http` module runs inside the daemon process alongside adapters. It exposes 4 JSON endpoints on localhost. The CLI reads a port file (`~/.viben/api.port`) to discover the server and uses `fetch()` to call it.
...
- [ ] **Step 4: Add VIBEN_API_PORT env override**
...
**Context:** This is the main feature module. It creates an HTTP server that listens on localhost, handles 4 endpoints, manages the port file (`~/.viben/api.port`), and interacts with `VibenCore` to create/cancel/list sessions and agents.

Key details from the codebase:
- `VibenCore` has `sessionManager`, `agentManager`, `configManager` as public properties
...
describe('ApiServer', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks()
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'viben-api-test-'))
...
**Context:** The CLI entry point is `src/cli.ts`. Commands are handled via `if (command === '...')` blocks. The `api` subcommand reads `~/.viben/api.port` to discover the daemon's API server, then uses `fetch()` to call it.
...
describe('api-client', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'viben-cli-api-'))
...
Runtime (requires running daemon):
  viben api new [agent]       Create a new session
  ...
  if (command === 'api') {
    ...
    try {
      if (subCmd === 'new') {
        ...
        console.log('Usage:')
        console.log('  viben api new [agent]         Create a new session')
