...
**Goal:** Add `viben config` (menu-based config editor) and daemon mode with auto-start on boot, including onboarding integration.
...
Also add `VIBEN_RUN_MODE` to `applyEnvOverrides`:
...
describe('daemon', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'viben-daemon-test-'))
    pidFile = path.join(tmpDir, 'viben.pid')
  })
...
export function startDaemon(pidPath: string = DEFAULT_PID_PATH, logDir?: string): { pid: number } | { error: string } {
  ...
  const logFile = path.join(resolvedLogDir, 'viben.log')
...
describe('autostart', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'viben-autostart-test-'))
  })
  ...
  describe('generateLaunchdPlist', () => {
    it('generates valid plist with absolute paths', async () => {
      ...
      const plist = generateLaunchdPlist('/usr/local/bin/node', '/usr/local/lib/cli.js', '/Users/test/.viben/logs')
      expect(plist).toContain('/usr/local/bin/node')
      expect(plist).toContain('/usr/local/lib/cli.js')
      expect(plist).toContain('--daemon-child')
      expect(plist).toContain('com.viben.daemon')
...
const LAUNCHD_LABEL = 'com.viben.daemon'
const LAUNCHD_PLIST_PATH = path.join(os.homedir(), 'Library', 'LaunchAgents', `${LAUNCHD_LABEL}.plist`)
const SYSTEMD_SERVICE_PATH = path.join(os.homedir(), '.config', 'systemd', 'user', 'viben.service')
...
export function generateLaunchdPlist(nodePath: string, cliPath: string, logDir: string): string {
  const logFile = path.join(logDir, 'viben.log')
...
export function installAutoStart(logDir: string): { success: boolean; error?: string } {
  ...
  try {
    ...
    if (process.platform === 'linux') {
      ...
      execFileSync('systemctl', ['--user', 'enable', 'viben'], { stdio: 'pipe' })
  ...
  try {
    ...
    if (process.platform === 'linux') {
      if (fs.existsSync(SYSTEMD_SERVICE_PATH)) {
        try {
          execFileSync('systemctl', ['--user', 'disable', 'viben'], { stdio: 'pipe' })
        } catch {
...
export async function runConfigEditor(configManager: ConfigManager): Promise<void> {
  ...
  try {
    while (true) {
      console.log(`\n${c.cyan}${c.bold}  Viben Configuration${c.reset}\n`)
    ...
    if (Object.keys(updates).length > 0) {
      ...
      console.log(dim('Restart Viben for changes to take effect.'))
    } else {
...
function printHelp(): void {
  console.log(`
Viben - Self-hosted bridge for AI coding agents
...
```typescript
  ...
  if (command === 'status') {
    ...
    if (status.running) {
      console.log(`Viben is running (PID ${status.pid})`)
    } else {
      console.log('Viben is not running')
    }
    return
  }
...
export async function setupRunMode(): Promise<{ runMode: 'foreground' | 'daemon'; autoStart: boolean }> {
    ...
    choices: [
      {
        ...
        description: 'Runs silently, auto-starts on boot. Manage with: viben status | stop | logs',
      },
      {
        name: 'Foreground (terminal)',
        value: 'foreground' as const,
        description: 'Runs in current terminal session. Start with: viben',
      },
    ],
  })
  ...
  if (mode === 'daemon') {
    ...
    if (autoStart) {
      const result = installAutoStart(expandHome('~/.viben/logs'))
      if (result.success) {
...
Expected: "Viben is not running" (since we haven't started a daemon)
