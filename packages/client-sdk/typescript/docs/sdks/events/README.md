...
<!-- UsageSnippet language="typescript" operationID="events_list" method="get" path="/api/events" -->
```typescript
...
import { eventsList } from "@viben/client-sdk/funcs/eventsList.js";
...
async function run() {
  const res = await eventsList(vibenClient);
  if (res.ok) {
