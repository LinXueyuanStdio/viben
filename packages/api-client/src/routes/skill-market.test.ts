...
describe("Skill Market Proxy Routes", () => {
  let app: FastifyInstance;

  beforeEach(async () => {
    vi.clearAllMocks();
    app = Fastify({ logger: false });
    registerSkillMarketProxyRoutes(app, {
  ...
  describe("GET /api/skill-market/search", () => {
    it("returns search results", async () => {
      ...
      const res = await app.inject({
        method: "GET",
        url: "/api/skill-market/search?q=test",
      });
      expect(res.statusCode).toBe(200);
      const body = JSON.parse(res.body);
      expect(body.data).toHaveLength(1);
  ...
  describe("GET /api/skill-market/categories", () => {
    it("returns categories", async () => {
      ...
      const res = await app.inject({
        method: "GET",
        url: "/api/skill-market/categories",
      });
      expect(res.statusCode).toBe(200);
  ...
  describe("GET /api/skill-market/:id", () => {
    it("returns package details by id", async () => {
      mocks.get.mockResolvedValue({ id: "pkg1", name: "test-skill", description: "A test skill", version: "2.0.0", author: { username: "creator" } });
      const res = await app.inject({
        method: "GET",
        url: "/api/skill-market/pkg1",
      });
      expect(res.statusCode).toBe(200);
      const body = JSON.parse(res.body);
    ...
    it("returns 404 on client error", async () => {
      ...
      const res = await app.inject({
        method: "GET",
        url: "/api/skill-market/nonexistent",
      });
      expect(res.statusCode).toBe(404);
    });
  });

  describe("GET /api/skill-market/:id/download", () => {
    it("returns binary response", async () => {
      ...
      const res = await app.inject({
        method: "GET",
        url: "/api/skill-market/pkg1/download",
      });
      expect(res.statusCode).toBe(200);
  ...
  describe("GET /api/skill-market/:id/comments", () => {
    it("returns comments", async () => {
      ...
      const res = await app.inject({
        method: "GET",
        url: "/api/skill-market/pkg1/comments",
      });
      expect(res.statusCode).toBe(200);
  ...
  describe("POST /api/skill-market/:id/comments", () => {
  ...
  describe("POST /api/skill-market/:id/rating", () => {
    it("returns success with valid score", async () => {
      mocks.rate.mockResolvedValue({ success: true, averageScore: 4.5, totalRatings: 8 });
      const res = await app.inject({
        method: "POST",
        url: "/api/skill-market/pkg1/rating",
        headers: {
          Authorization: "Bearer bmcp_12345678_abcdefghijklmnopqrstuvwx",
        },
        payload: { score: 4 },
      ...
      expect(body.averageScore).toBe(4.5);
      expect(body.totalRatings).toBe(8);
    });
