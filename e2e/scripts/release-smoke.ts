...
async function main(): Promise<void> {
  ...
  const namespace = process.env.VIBEN_PACKAGED_E2E_NAMESPACE ?? defaultNamespace(platform);
  const reportRoot = resolveFromWorkspace(
    process.env.VIBEN_PACKAGED_E2E_REPORT_DIR ?? join('.tmp', 'release-report', platform),
  );
  const report = await createReport(reportRoot);

  process.env.VIBEN_PACKAGED_E2E_REPORT_DIR = report.root;

  await report.json('manifest.json', {
    ...(process.env.VIBEN_PACKAGED_E2E_RELEASE_CHANNEL == null
      ? {}
      : { channel: process.env.VIBEN_PACKAGED_E2E_RELEASE_CHANNEL }),
    ...(process.env.VIBEN_PACKAGED_E2E_RELEASE_VERSION == null
      ? {}
      : { releaseVersion: process.env.VIBEN_PACKAGED_E2E_RELEASE_VERSION }),
    ...
    screenshot: `screenshots/viben-${platform}-smoke.png`,
    spec,
  });
  await saveRequiredSource(report, 'tools-pack.json', process.env.VIBEN_PACKAGED_E2E_BUILD_JSON_PATH);
  await saveOptionalSource(report, 'tools-pack.log', process.env.VIBEN_PACKAGED_E2E_BUILD_LOG_PATH);
