...
describe('namespace isolation spec', () => {
  test('keeps namespace in lifecycle infrastructure while daemon clients use URL or concrete IPC transport', async () => {
    ...
    await suite.with.toolsDev(async ({ runtime, status, webUrl }) => {
      ...
      const ipcPath = installInfo.env.VIBEN_SIDECAR_IPC_PATH;
      if (typeof ipcPath !== 'string' || ipcPath.length === 0) {
        throw new Error('MCP install-info did not include VIBEN_SIDECAR_IPC_PATH');
      }
      expect(ipcPath).toEqual(expect.any(String));
      expect(installInfo.args).not.toContain('--daemon-url');
      expect(installInfo.env.VIBEN_DATA_DIR).toBe(suite.dataDir);
      expect(installInfo.env).not.toHaveProperty('VIBEN_NAMESPACE');
      expect(installInfo.env).not.toHaveProperty('VIBEN_SIDECAR_NAMESPACE');
      expect(installInfo.env).not.toHaveProperty('VIBEN_SIDECAR_IPC_BASE');

      const cliStatus = await runDaemonCliJson<DaemonStatusResponse>(
        ['daemon', 'status', '--json'],
        {
          VIBEN_DATA_DIR: suite.dataDir,
          VIBEN_NAMESPACE: 'wrong-daemon-namespace',
          VIBEN_SIDECAR_IPC_BASE: path.join(suite.scratchDir, 'wrong-ipc-base'),
          VIBEN_SIDECAR_IPC_PATH: ipcPath,
          VIBEN_SIDECAR_NAMESPACE: 'wrong-sidecar-namespace',
        },
      );
      ...
      await access(path.join(expectedPluginRoot, 'viben.json'));
...
async function writeLocalPluginFixture(root: string): Promise<string> {
  ...
  await writeFile(
    path.join(pluginRoot, 'viben.json'),
    JSON.stringify(
      {
...
async function runDaemonCli(
  ...
  delete mergedEnv.VIBEN_DAEMON_URL;
  delete mergedEnv.VIBEN_PORT;
