...
const toolsPackDir = resolveFromWorkspace(process.env.VIBEN_PACKAGED_E2E_TOOLS_PACK_DIR ?? '.tmp/tools-pack');
const namespace = process.env.VIBEN_PACKAGED_E2E_NAMESPACE ?? 'release-beta-win';
...
const maxInstallDurationMs = Number.parseInt(process.env.VIBEN_PACKAGED_E2E_WIN_MAX_INSTALL_MS ?? '120000', 10);
const verifyReinstallWhileRunning = process.env.VIBEN_PACKAGED_E2E_WIN_VERIFY_REINSTALL !== '0';
const verifyViolentUpdater = process.env.VIBEN_PACKAGED_E2E_WIN_UPDATER_VIOLENT !== '0';
const verifyRealUpdateInstaller = process.env.VIBEN_PACKAGED_E2E_WIN_REAL_UPDATE_INSTALL === '1';
const releaseChannel = process.env.VIBEN_PACKAGED_E2E_RELEASE_CHANNEL;
const releaseVersion = process.env.VIBEN_PACKAGED_E2E_RELEASE_VERSION;
...
const violentArtifactBytes = Number.parseInt(process.env.VIBEN_PACKAGED_E2E_WIN_UPDATER_BYTES ?? `${16 * 1024 * 1024}`, 10);
const violentChunkDelayMs = Number.parseInt(process.env.VIBEN_PACKAGED_E2E_WIN_UPDATER_CHUNK_DELAY_MS ?? '18', 10);
...
const startUpdaterDownloadExpression = `
  (() => {
    const updater = window.__viben__?.updater;
...
const shouldRunPackagedWinSmoke = process.platform === 'win32' && process.env.VIBEN_PACKAGED_E2E_WIN === '1';
...
winDescribe('packaged windows runtime smoke', () => {
  ...
  test('installs, starts, inspects with eval and screenshot, stops, and uninstalls the built windows artifact', async () => {
    ...
    try {
      ...
      expect(JSON.stringify(install.registryEntries)).toContain(`Viben-${installIdentity.namespaceToken}`);
      ...
      expect(inspect.status?.url).toBe('viben://app/');

      const value = assertHealthEvalValue(inspect.eval?.value);
      expect(value.href).toBe('viben://app/');
...
async function runRealUpdateInstallerAcceptance(options: {
  ...
  if (releaseChannel == null || releaseChannel === '' || releaseVersion == null || releaseVersion === '') {
    throw new Error('VIBEN_PACKAGED_E2E_WIN_REAL_UPDATE_INSTALL requires VIBEN_PACKAGED_E2E_RELEASE_CHANNEL and VIBEN_PACKAGED_E2E_RELEASE_VERSION');
  }
  ...
  applyManualPackagedUpdateEnv(
    process.env,
    updateScenario,
    `https://releases.viben.ai/${updateScenario.channel}/latest/metadata.json`,
  );
  ...
  expect(entry.keyPath).toContain(`Viben-${installIdentity.namespaceToken}`);
...
async function ensureManagedDownloadBase(updateRoot: string): Promise<string> {
  ...
  await writeFile(
    join(downloadsRoot, '.viben-download-root.json'),
    `${JSON.stringify({
      createdAt: new Date().toISOString(),
      kind: 'viben-managed-download-root',
      schemaVersion: 1,
    }, null, 2)}\n`,
    'utf8',
  );
...
const UPDATE_ENV_KEYS = [
  'VIBEN_UPDATE_AUTO_CHECK',
  'VIBEN_UPDATE_AUTO_DOWNLOAD',
  'VIBEN_UPDATE_ENABLED',
  'VIBEN_UPDATE_METADATA_URL',
  'VIBEN_UPDATE_CURRENT_VERSION',
  'VIBEN_UPDATE_OPEN_DRY_RUN',
] as const;
...
function applyManualPackagedUpdateEnv(
  ...
  env.VIBEN_UPDATE_AUTO_CHECK = '0';
  env.VIBEN_UPDATE_AUTO_DOWNLOAD = '0';
}
...
async function startViolentUpdaterFixtureProcess(
  ...
  const artifactName = `viben-${version}-win-x64-setup.exe`;
...
async function resolveExpectedNamespaceRoot(installDir: string): Promise<string> {
  const installedConfig = JSON.parse(
    await readFile(join(installDir, 'resources', 'viben-config.json'), 'utf8'),
