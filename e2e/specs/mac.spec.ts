...
const namespace = process.env.VIBEN_PACKAGED_E2E_NAMESPACE ?? 'release-beta';
...
const pnpmCommand = process.env.VIBEN_E2E_PNPM_COMMAND ?? 'pnpm';
...
const shouldRunPackagedMacSmoke = process.platform === 'darwin' && process.env.VIBEN_PACKAGED_E2E_MAC === '1';
const macDescribe = shouldRunPackagedMacSmoke ? describe : describe.skip;
const shouldRunDesktopMacSmoke = process.platform === 'darwin' && process.env.VIBEN_DESKTOP_SMOKE === '1';
...
macDescribe('packaged mac runtime smoke', () => {
  ...
  test('installs, starts, inspects, stops, and uninstalls the built mac artifact', async () => {
    ...
    try {
      ...
      expect(inspect.status?.url).toMatch(/^(viben:\/\/app\/|http:\/\/127\.0\.0\.1:\d+\/)/);

      const value = assertHealthEvalValue(inspect.eval?.value);
      expect(value.href).toMatch(/^(viben:\/\/app\/|http:\/\/127\.0\.0\.1:\d+\/)/);
  ...
  test('clicking the Orbit Open artifact link keeps the desktop settings dialog stable', async () => {
    ...
    await desktop.eval(`
      (() => {
        ...
        window.__vibenLastOpenArtifactHref = null;
        window.__vibenOpenArtifactClickCount = 0;
        if (!window.__vibenOpenArtifactClickCaptureInstalled) {
          document.addEventListener('click', (event) => {
            const target = event.target instanceof Element ? event.target.closest('a') : null;
            if (!(target instanceof HTMLAnchorElement)) return;
            if (target.textContent?.trim() !== 'Open artifact') return;
            window.__vibenLastOpenArtifactHref = target.getAttribute('href');
            window.__vibenOpenArtifactClickCount += 1;
            event.preventDefault();
          }, true);
          window.__vibenOpenArtifactClickCaptureInstalled = true;
        }
        return true;
      })()
    `);
    ...
    const clickCapture = await desktop.eval<{ count: number; href: string | null }>(`
      (() => ({
        count: typeof window.__vibenOpenArtifactClickCount === 'number' ? window.__vibenOpenArtifactClickCount : 0,
        href: typeof window.__vibenLastOpenArtifactHref === 'string' ? window.__vibenLastOpenArtifactHref : null,
      }))()
    `);
  ...
  test('keeps the desktop workspace stable when the artifact Open link is clicked', async () => {
    ...
    const seeded = await desktop.eval<{ projectId: string }>(`
      (async () => {
        ...
        window.__vibenDesktopOpenHref = null;
        window.__vibenDesktopOpenClickCount = 0;
        if (!window.__vibenDesktopOpenCaptureInstalled) {
          document.addEventListener('click', (event) => {
            const target = event.target instanceof Element ? event.target.closest('a') : null;
            if (!(target instanceof HTMLAnchorElement)) return;
            if (target.textContent?.trim() !== 'Open') return;
            window.__vibenDesktopOpenHref = target.getAttribute('href');
            window.__vibenDesktopOpenClickCount += 1;
            event.preventDefault();
          }, true);
          window.__vibenDesktopOpenCaptureInstalled = true;
        }
    ...
    const clickCapture = await desktop.eval<{ count: number; href: string | null }>(`
      (() => ({
        count: typeof window.__vibenDesktopOpenClickCount === 'number' ? window.__vibenDesktopOpenClickCount : 0,
        href: typeof window.__vibenDesktopOpenHref === 'string' ? window.__vibenDesktopOpenHref : null,
      }))()
    `);
    expect(clickCapture.count).toBeGreaterThan(0);
