...
export async function createFakeAgentRuntimes(
  ...
  const root = Array.isArray(input)
    ? path.join(tmpdir(), `viben-fake-agents-${process.pid}`)
    : (input.root ?? path.join(tmpdir(), `viben-fake-agents-${process.pid}`));
...
async function emitRun(promptText) {
  ...
  if (
    promptText.includes('Create a Viben plugin for:') &&
...
async function emitPluginAuthoringRun() {
  ...
  const summary = [
    'Created generated-plugin with viben.json, SKILL.md, and examples/demo.md.',
    'viben plugin validate: passed',
    'viben plugin pack: generated-plugin-0.1.0.tgz',
    'viben plugin install --source: passed',
  ].join('\\n');
...
async function createOrbitLiveArtifact() {
  const baseUrl = process.env.VIBEN_DAEMON_URL;
  const token = process.env.VIBEN_TOOL_TOKEN;
  if (!baseUrl || !token) {
    throw new Error('Orbit fake run requires VIBEN_DAEMON_URL and VIBEN_TOOL_TOKEN');
  }
