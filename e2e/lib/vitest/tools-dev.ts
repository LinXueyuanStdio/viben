...
async function runToolsDevJson<T>(suite: SmokeSuite, args: string[]): Promise<T> {
  const useNpmExecPathWithNode = process.env.VIBEN_E2E_PNPM_COMMAND == null
    && pnpmExecPath != null
    && nodeLoadablePackageManagerExtensions.has(extname(pnpmExecPath).toLowerCase());
  const command = useNpmExecPathWithNode
    ? process.execPath
    : (process.env.VIBEN_E2E_PNPM_COMMAND == null && pnpmExecPath ? pnpmExecPath : pnpmCommand);
  ...
  const { stdout } = await execFileAsync(command, commandArgs, {
    ...
    env: {
      ...
      VIBEN_MEDIA_CONFIG_DIR: suite.dataDir,
    },
