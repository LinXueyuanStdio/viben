...
function displayNameForChannel(channel: ReleaseChannelIdentity): string {
  if (channel === "beta") return "Viben Beta";
  if (channel === "nightly") return "Viben Nightly";
  if (channel === "preview") return "Viben Preview";
  return "Viben";
}
...
export function resolvePackagedWinInstallIdentity(options: {
  ...
  const displayName = channel == null ? `Viben ${namespaceToken}` : displayNameForChannel(channel);
