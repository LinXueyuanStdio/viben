...
const GITHUB_STARS_STORAGE_KEY = 'viben:gh-stars';
const VISUAL_STYLE_ID = 'viben-visual-stability-style';
// Keep this exact-route mock narrow so unrelated GitHub UI still behaves normally.
const VISUAL_GITHUB_REPO_API = 'https://api.github.com/repos/nexu-io/viben';
...
const VISUAL_PLUGINS = [
  ...
  makeVisualPlugin({
    ...
    description: 'Migrate a Figma frame into an editable Viben project.',
    mode: 'prototype',
...
export async function waitForVisualReady(page: Page): Promise<void> {
  await page.getByText('Loading Viben…').waitFor({ state: 'detached', timeout: 10_000 }).catch(() => {});
...
export async function captureVisual(page: Page, name: string): Promise<string> {
  const outputDir = path.resolve(process.env.VIBEN_VISUAL_OUTPUT_DIR || 'ui/reports/visual-screenshots');
