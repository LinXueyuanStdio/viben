...
test('sending preview comments keeps the preview live and refreshes it with the follow-up artifact', async ({ page }) => {
  ...
  await page.route('**/api/runs/*/events', async (route) => {
    const revisedHtml =
      '<!doctype html><html><body><main data-viben-id="hero-section">' +
      '<h1 data-viben-id="hero-title" data-screen-label="Hero title">Revised headline</h1>' +
      '<p data-viben-id="hero-copy">Preview copy refreshed after comment send.</p>' +
      '</main></body></html>';
...
async function waitForLoadingToClear(page: Page) {
  await page.getByText('Loading Viben…').waitFor({ state: 'hidden', timeout: T.medium });
}
