...
describe('packaged updater release scenario', () => {
  ...
  test('does not override current version for release-channel smoke', () => {
    const env: NodeJS.ProcessEnv = {
      VIBEN_UPDATE_CURRENT_VERSION: '99.0.0-beta.0',
    };
    ...
    expect(env.VIBEN_UPDATE_CURRENT_VERSION).toBeUndefined();
    expect(env.VIBEN_UPDATE_METADATA_URL).toBe('http://127.0.0.1:1234/nightly/latest/metadata.json');
    expect(env.VIBEN_UPDATE_AUTO_CHECK).toBe('1');
  });
});
