...
describe("packaged smoke workflow", () => {
  it("keeps packaged smoke outside the main CI gate", async () => {
    ...
    expect(workflow).not.toContain("VIBEN_PACKAGED_E2E_");
  ...
  it("preserves beta linux AppImage smoke reports for platform publication", async () => {
    ...
    expect(workflow).toContain("viben-beta-linux-e2e-report");
    ...
    expect(workflow).toContain("viben-beta-linux-publish-manifest");
  ...
  it("preserves stable linux AppImage smoke reports for release publication", async () => {
    ...
    expect(workflow).toContain("tools-pack.json");
    expect(workflow).toContain("Upload linux e2e spec report");
    expect(workflow).toContain("viben-release-linux-e2e-report");
  ...
  it("keeps release namespaces aligned with release channels", async () => {
    ...
    expect(releaseStableWorkflow).toContain("VIBEN_PACKAGED_E2E_NAMESPACE: ${{ needs.metadata.outputs.namespace }}");
    ...
    expect(releaseStableWorkflow).toContain('VIBEN_PACKAGED_E2E_NAMESPACE: ${{ needs.metadata.outputs.win_namespace }}');
    ...
    expect(releaseStableWorkflow).not.toMatch(/VIBEN_PACKAGED_E2E_NAMESPACE: release-stable(?:-win|-linux)?\b/);
    ...
    expect(releaseBetaWorkflow).toContain("VIBEN_PACKAGED_E2E_RELEASE_CHANNEL: beta");
    expect(releaseBetaWorkflow).toContain("VIBEN_PACKAGED_E2E_RELEASE_VERSION: ${{ needs.metadata.outputs.beta_version }}");
  });
});
...
function expectChannelWorkflowNamespaces(
  ...
  expect(workflow).toContain(`VIBEN_PACKAGED_E2E_NAMESPACE: ${namespace}`);
  ...
  expect(workflow).toContain(`VIBEN_PACKAGED_E2E_NAMESPACE: ${namespace}-win`);
  ...
  if (options.hasLinuxSmoke) {
    expect(workflow).toContain(`VIBEN_PACKAGED_E2E_NAMESPACE: ${namespace}-linux`);
  }
}
