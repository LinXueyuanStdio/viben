#!/bin/bash
# Generate GitHub Job Summary for Linux Desktop E2E test results
...
echo "## Linux Desktop E2E 测试报告" >> "$SUMMARY_FILE"
echo "" >> "$SUMMARY_FILE"
...
echo "|------|-----|" >> "$SUMMARY_FILE"
echo "| 系统 | Linux x86_64 |" >> "$SUMMARY_FILE"
echo "| 测试框架 | WebdriverIO + tauri-driver |" >> "$SUMMARY_FILE"
echo "| 显示 | Xvfb (1920x1080x24) |" >> "$SUMMARY_FILE"
...
# Compress images with ImageMagick if available
TEMP_DIR=$(mktemp -d)
...
for img in "${SCREENSHOTS[@]}"; do
  ...
  if command -v convert &>/dev/null; then
    convert "$img" -resize "600x>" -quality 85 "$TEMP_DIR/$name" 2>/dev/null || cp "$img" "$TEMP_DIR/$name"
  else
...
# Upload screenshots
UPLOAD_DIR="linux/${GITHUB_RUN_ID}"
