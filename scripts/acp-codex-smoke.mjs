...
const provider = args.provider ?? process.env.CODEX_SMOKE_PROVIDER;
const baseUrl = args["base-url"] ?? args.baseUrl ?? process.env.CODEX_SMOKE_BASE_URL;
...
const sandbox = args.sandbox ?? "readOnly";
if (baseUrl && !provider) {
  throw new Error("--base-url requires --provider so the override can target an existing Codex provider");
}
...
async function main() {
  ...
  try {
    ...
    if (updateKinds.includes("codex_event")) {
      throw new Error(`Unexpected raw codex_event update; updates=${updateKinds.join(",")}`);
    }
