...
**The Solution**: The `/viben:finish-work` command re-verifies code against guidelines AFTER writing, catching drift that occurred during development and doing a final holistic review.
...
**WHAT IT ACTUALLY DOES**:
1. Re-reads the guidelines from `docs/specs/`
2. Compares written code against those guidelines
3. Runs type checker and linter
4. Reviews all changes holistically
5. Checks cross-layer consistency
6. Identifies broader impacts
7. Checks if new patterns should be documented
...
**[3/6] Read docs/specs/frontend/** - Read project-specific frontend knowledge
**[4/6] Investigate and fix the bug** - Actual development work
**[5/6] /viben:finish-work** - Re-verify code and holistic cross-layer review
**[6/6] Human tests and commits, then /viben:record-session** - Human validates and persist memory
...
**[1/5] /viben:start** - Resume context from previous session
**[2/5] Read docs/specs/backend/** - Re-read guidelines before fixes
**[3/5] Fix each CR issue** - Address feedback with guidelines in context
**[4/5] /viben:finish-work** - Verify fixes and document lessons from CR
**[5/5] Human commits, then /viben:record-session** - Preserve CR lessons
...
**[5/5] Human commits, then /viben:record-session** - Link all commits to one feature

### Example 5: Debug Session

**[1/5] /viben:start** - See if this bug was investigated before
**[2/5] Read docs/specs/backend/** - Guidelines might document known gotchas
**[3/5] Investigation** - Actual debugging work
**[4/5] /viben:finish-work** - Verify changes and document debug findings
**[5/5] Human commits, then /viben:record-session** - Debug knowledge is valuable
...
2. **Guidelines before code** - Read `docs/specs/` before coding.
3. **Check after code** - /viben:finish-work catches context drift.
