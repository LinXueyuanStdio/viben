...
**原则二：数据分层。** page HTML 是唯一阻塞数据（T1），其余全部流式或客户端延迟加载（T2/T3）。
...
T0 (0ms, 客户端同步)
  浏览器解析 URL
    → isPublishedPageRoute(pathname) → 是 page
    → session.userSlug === urlUserSlug → isAuthor
    → Topbar 立即渲染：阅读布局 + Page tab + (isAuthor ? Settings tab) + 右侧按钮
    → 仅 Side Page tab 等服务端确认

T1 (~10-30ms, 服务端阻塞)
  SELECT * FROM published_pages WHERE author_slug = ? AND uid = ?
    → page HTML + authorName + authorAvatarUrl + sidePageUid
    → 立即流式返回 ReadPageShell + iframe srcDoc
    → Side Page tab 出现（如果 sidePageUid != null）

T2 (流式, +10-40ms)
  Suspense: getCommunitySummary() → 点赞/收藏数

T3 (客户端闲时)
  requestIdleCallback: prefetchQuery 评论 + 笔记
  dynamic(): Drawer tabs、Settings panel 按需加载
```
...
export function Topbar({ session, centerContent, rightContent, ... }: TopbarProps) {
  ...
  // 插槽优先级：props > context > 默认
  const effectiveCenter = centerContent ?? slots?.centerContent ?? (
    isReadPageFromUrl ? tablist : <GlobalSearch ... />
  )
