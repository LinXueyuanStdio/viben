...
let healthzBody: string | null = null;
...
const execMock = mock(async (command: string) => {
  ...
  if (command === "pgrep -f code-server") {
    ...
    const pids = processListOutput
      .split("\n")
      .filter((line) => line.includes("code-server"))
      .map((line) => line.trim().match(/^\s*([1-9][0-9]*)/)?.[1])
      .filter((pid): pid is string => pid !== undefined && runningPids.has(pid));
    return pids.length > 0 ? successResult(pids.join("\n")) : failureResult("no match");
  }
  ...
  if (command.includes("curl -s --max-time 2 http://127.0.0.1:8000/healthz")) {
    if (healthzBody !== null) {
      return successResult(healthzBody);
    }
...
describe("/api/sessions/[sessionId]/code-editor", () => {
  ...
  test("POST returns a conflict instead of opening an unrelated app on the editor port", async () => {
    ...
    runningPids.add("4321");
    portProbeStatusCode = "200";
    healthzBody = null; // Not code-server healthz
  ...
  test("POST launches code-server when the port is free", async () => {
    ...
    expect(lastLaunchCommand).toContain("code-server");
    expect(lastLaunchCommand).toContain("--port 8000");
  ...
  test("POST recovers from stale lock when code-server is already running", async () => {
    ...
    runningPids.add(RUNNING_CODE_SERVER_PID);
    processListOutput =
      ` ${RUNNING_CODE_SERVER_PID} code-server --port 8000 --auth none --bind-addr 0.0.0.0:8000 /vercel/sandbox\n`;
    ...
    expect(directories.has(CODE_EDITOR_LOCK_DIR)).toBe(true);
  ...
  test("POST recovers from stale lock and launches when port is free", async () => {
    ...
    expect(directories.has(CODE_EDITOR_LOCK_DIR)).toBe(false);
    expect(execDetachedMock).toHaveBeenCalledTimes(1);
  });
});
