...
async function seed() {
  ...
  const demoPages = [
    {
      ...
      authorDisplayName: demoAuthor.displayName,
      authorAvatarUrl: null as string | null,
      authorSlug: demoAuthor.userSlug,
    ...
    },
    {
      ...
      authorDisplayName: bob.displayName,
      authorAvatarUrl: null as string | null,
      authorSlug: bob.userSlug,
      visibility: "public" as const,
      moderationStatus: "approved" as const,
      viewCount: 2100,
      likeCount: 102,
    ...
    if (existing) {
      ...
      const [p] = await db
        ...
        .values({
          ...
          bookmarkCount: dp.bookmarkCount,
          ...
          tags: [],
          coverUrl: null,
          authorDisplayName: dp.authorDisplayName,
          authorAvatarUrl: dp.authorAvatarUrl,
          authorSlug: dp.authorSlug,
          sidePageUid: null,
          chaptersJson: null,
  ...
  const pageEntities = createdPages.map((p) => ({
    ...
    authorDisplayName: p.authorDisplayName ?? null as string | null,
  }));
  ...
  for (const user of allUsers) {
    for (let j = 0; j < 5; j++) {
      ...
      await db.insert(userBrowseHistory).values({
        ...
        snapshotAuthorName: page.authorDisplayName,
