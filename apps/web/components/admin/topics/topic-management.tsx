...
export function TopicManagement() {
  ...
  const [filter, setFilter] = useState<'all' | 'featured' | 'blocked'>('all');
  ...
  const fetchTopics = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/topics?filter=${filter}`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let res: Response;
      if (editingTopic) {
        res = await fetch(`/api/admin/topics/${editingTopic.id}`, {
          method: 'PATCH',
  ...
  const handleToggleFeatured = async (topic: Topic) => {
    ...
    try {
      const res = await fetch(`/api/admin/topics/${topic.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ is_featured: !topic.isFeatured }),
      });
  ...
  const handleToggleBlocked = async (topic: Topic) => {
    setTogglingId(topic.id);
    try {
      const res = await fetch(`/api/admin/topics/${topic.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ is_blocked: !topic.isBlocked }),
      });
      if (!res.ok) throw new Error('Failed to update topic');
      fetchTopics();
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <button onClick={fetchTopics} className="mt-2 text-sm text-primary hover:underline">
            重试
          </button>
        </div>
      ) : topics.length === 0 ? (
                ...
                <tr key={topic.id} className="border-b last:border-0 hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3 text-sm">
                    <div className="flex gap-1">
                      ...
                      {!topic.isFeatured && !topic.isBlocked && (
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <Button
                        ...
                        title={topic.isFeatured ? '取消精选' : '设为精选'}
                      >
                        <Star
                          className={`h-4 w-4 ${
                            topic.isFeatured ? 'fill-yellow-400 text-yellow-400' : ''
                          }`}
                        />
                      </Button>
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => handleToggleBlocked(topic)}
                        disabled={togglingId === topic.id}
                        title={topic.isBlocked ? '取消屏蔽' : '屏蔽'}
                      >
                        <ShieldOff
                          className={`h-4 w-4 ${
                            topic.isBlocked ? 'text-destructive' : ''
                          }`}
                        />
                      </Button>
                      <Button
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <Label htmlFor="isFeatured">设为精选</Label>
              <Switch
                id="isFeatured"
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave} disabled={saving || !formDisplayName || !formSlug}>
