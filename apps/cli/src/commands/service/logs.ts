 ...
 * View service logs
 */
export function showServiceLogs(ctx: OutputContext, options: LogsOptions): void {
  const { name, follow, lines = 100, clear } = options;

  // Clear logs if requested
  if (clear) {
    const { clearServiceLogs } = require('../../lib/services');
  ...
  if (follow) {
    ...
    if (ctx.json) {
      ...
      const logs = readServiceLogs(name, lines);
      console.log(
        JSON.stringify(
          {
            success: true,
    ...
    const existingLogs = readServiceLogs(name, lines);
