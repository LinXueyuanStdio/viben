...
/**
 * Get the validation error message for a metadata key, or null if valid.
 *
 * @param key - Full metadata key entered by the user.
 * @returns Error message string if invalid, null if valid.
 */
export const getMetaKeyValidationError = (key: string): string | null => {
  const trimmedKey = key.trim();
  if (!trimmedKey) return null;

  if (!hasValidMetaPrefix(trimmedKey)) {
    return META_PREFIX_RULES_MESSAGE;
  }

  if (isReservedMetaKey(trimmedKey)) {
    return RESERVED_NAMESPACE_MESSAGE;
  }

  if (!hasValidMetaName(trimmedKey)) {
    return META_NAME_RULES_MESSAGE;
  }
