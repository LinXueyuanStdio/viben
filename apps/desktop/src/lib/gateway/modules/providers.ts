...
export async function createProvider(
  ...
  const response = await fetch(`${baseUrl}/api/providers`, {
    ...
    body: JSON.stringify({
      type: options.type,
      name: options.name,
      api_key: options.api_key ?? options.apiKey,
      base_url: options.base_url ?? options.baseUrl,
      api_version: options.api_version ?? options.apiVersion,
      deployment: options.deployment,
      timeout: options.timeout,
      max_retries: options.max_retries ?? options.maxRetries,
      headers: options.headers,
      category: options.category,
      surfaces: options.surfaces,
      supports_custom_model: options.supports_custom_model ?? options.supportsCustomModel,
      set_as_default: options.set_as_default ?? options.setAsDefault,
    }),
  });
  ...
  const response = await fetch(
    `${baseUrl}/api/providers/${encodeURIComponent(providerId)}`,
    {
      ...
      body: JSON.stringify({
        type: updates.type,
        name: updates.name,
        api_key: updates.api_key ?? updates.apiKey,
        base_url: updates.base_url ?? updates.baseUrl,
        api_version: updates.api_version ?? updates.apiVersion,
        deployment: updates.deployment,
        timeout: updates.timeout,
        max_retries: updates.max_retries ?? updates.maxRetries,
        headers: updates.headers,
        category: updates.category,
        surfaces: updates.surfaces,
        supports_custom_model: updates.supports_custom_model ?? updates.supportsCustomModel,
      }),
    }
  );
