 ...
 */
export const useTerminalFontStore = create<TerminalFontSettingsStore>()(
  persist(
    (set, get) => ({
    ...
    }),
    {
      ...
      onRehydrateStorage: () => (state) => {
        ...
        if (!isValidFontFamily(state.fontFamily)) needsReset = true;
        if (!isValidFontSize(state.fontSize)) needsReset = true;
        if (!isValidFontWeight(state.fontWeight)) needsReset = true;
        if (!isValidLineHeight(state.lineHeight)) needsReset = true;
        if (!isValidLetterSpacing(state.letterSpacing)) needsReset = true;
        if (!isValidScrollback(state.scrollback)) needsReset = true;
        if (!isValidCursorStyle(state.cursorStyle)) needsReset = true;
        if (!isValidHexColor(state.cursorAccentColor)) needsReset = true;
        if (typeof state.cursorBlink !== 'boolean') needsReset = true;
