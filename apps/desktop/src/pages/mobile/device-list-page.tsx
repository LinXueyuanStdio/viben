    ...
    <div className="flex items-center gap-3 p-4 rounded-lg border bg-card">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs text-muted-foreground flex items-center gap-1">
          <span>{device.type === "gateway" ? t("mobile.devices.gateway", "Gateway") : t("mobile.devices.client", "Client")}</span>
          <span>·</span>
...
export function DeviceListPage() {
  ...
  if (!active) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-muted-foreground gap-2 p-6">
        ...
        <p>{t("mobile.devices.notConnected", "Not connected to any Gateway")}</p>
        <p className="text-sm">{t("mobile.devices.goToConnect", "Go to Connect to pair with your desktop")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 space-y-3">
      <h2 className="text-lg font-semibold">{t("mobile.devices.title", { count: devices.length, defaultValue: "Devices ({{count}})" })}</h2>
      {devices.length === 0 ? (
        <p className="text-sm text-muted-foreground">{t("mobile.devices.noDevices", "No devices found in the mesh.")}</p>
      ) : (
