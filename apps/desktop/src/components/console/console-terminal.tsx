  ...
  return defineCommand("gui", async (args) => {
    ...
    if (!actionName || actionName === "--help") {
      return {
        stdout: [
          ...
          "  desktop_main.list_navigation_paths List all navigable routes",
          "  desktop_main.current_window_state  Get current window state as text",
          "",
          "Presentation Actions (presentation.*):",
          "  Use `presentation` command directly for convenience.",
          "  Or: gui presentation.spotlight, gui presentation.chart, etc.",
          "",
          "Provider actions use namespace.name format.",
          "Run `gui list_actions` to see all registered actions.",
          "",
          "Examples:",
          "  gui list_actions",
    ...
    if (args.includes("--help")) {
      try {
        ...
        return { stdout: "", stderr: `gui ${actionName} --help: ${msg}\n`, exitCode: 1 };
      }
    }
    ...
    if (jsonIndex !== -1 && jsonIndex + 1 < args.length) {
      try {
        ...
        return { stdout: "", stderr: `gui: invalid JSON payload: ${err instanceof Error ? err.message : String(err)}\n`, exitCode: 1 };
      }
    }
    ...
    try {
      ...
      if (hasImage) {
        const textParts = result.content
          ...
          .filter(Boolean);
        return { stdout: `${textParts.join("\n")}\n`, stderr: "", exitCode: 0 };
      }

      return { stdout: `${JSON.stringify(resultToExecutorValue(result), null, 2)}\n`, stderr: "", exitCode: 0 };
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      return { stdout: "", stderr: `gui ${actionName}: ${msg}\n`, exitCode: 1 };
    }
  });
}
...
function createInputHandler(
  ...
  const executeCommand = async (command: string) => {
    ...
    if (trimmed === "clear") {
      ...
      if (result.stdout) term.write(formatMarkdown(colorizeUrls(result.stdout)).replace(/\n/g, "\r\n"));
  ...
  term.onData(async (e: string) => {
    ...
    if (e === "\x1b[A") { if (historyIndex > 0) { historyIndex--; setCmd(history[historyIndex]); } return; }
    if (e === "\x1b[B") {
      if (historyIndex < history.length - 1) { historyIndex++; setCmd(history[historyIndex]); }
      else if (historyIndex === history.length - 1) { historyIndex = history.length; setCmd(""); }
      return;
    }
    ...
    if (e === "\x1b[H" || e === "\x1bOH" || e === "\x1b[1~") { cursorPos = 0; redrawLine(); return; }
    if (e === "\x1b[F" || e === "\x1bOF" || e === "\x1b[4~") { cursorPos = cmd.length; redrawLine(); return; }
    ...
    if (e === "\x03") { term.writeln("^C"); cmd = ""; cursorPos = 0; term.write("$ "); return; }
...
export function ConsoleTerminal({ className }: ConsoleTerminalProps) {
  ...
  const initTerminal = useCallback(() => {
    ...
    const files = {
      "/home/user/README.md": "# Viben Console\n\nA sandboxed bash environment with AI agent and GUI action support.\n\nTry `agent hello` or `gui --help` to get started.\n",
