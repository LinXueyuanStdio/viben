...
export function acpSessionUpdateToUiSteps(notification: AcpSessionUpdate): AcpUiStep[] {
  ...
  switch (update.sessionUpdate) {
    ...
    case "tool_call_update": {
      ...
      // For Agent/Task tools, always update the tool_use with input when rawInput is available
      // This handles the case where the first tool_call has empty rawInput,
      // and the tool_call_update contains the actual description/subagent_type
      if (isToolInputOnlyUpdate(update) || (isAgentTool && update.rawInput && !isFinishedToolStatus(readString(update.status)))) {
        const steps: AcpUiStep[] = [];

        // Always emit tool_use update with the latest input
        steps.push(...messageStep("tool_use", {
          id: typeof update.toolCallId === "string" ? update.toolCallId : createStepId("tool"),
          type: "tool_use",
          name: toolName,
          toolUseId: update.toolCallId,
...
/**
 * Extract the actual tool name from an ACP session update.
 * For Agent/Task tools, the real tool name is in _meta.claudeCode.toolName,
 * while update.title contains the description.
 */
function extractToolName(update: Record<string, unknown>): string {
  const metaToolName = readClaudeCodeToolName(update._meta);
  if (metaToolName) return metaToolName;
  return normalizeToolName(update.title);
}
...
function readClaudeCodeToolName(meta: unknown): string | undefined {
  ...
  return readString(claudeCode.toolName);
}
