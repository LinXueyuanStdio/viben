...
export function InspectorPing({ makeRequest }: InspectorPingProps) {
    ...
    try {
      ...
      try {
        ...
        const successPing = {
          ...
          success: true,
          duration,
          method: t("inspector.fallbackMethod"),
        };
        ...
        const failedPing = {
          ...
          method: t("inspector.pingAndFallback"),
        };
  ...
  const getAverageResponseTime = () => {
    ...
    return Math.round(
      successfulPings.reduce((sum, ping) => sum + ping.duration, 0) / successfulPings.length
    );
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium">{t("inspector.pingTitle")}</span>
        </div>
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
          ...
          >
            {t("inspector.clearHistory")}
          </Button>
          <Button onClick={handlePing} disabled={pinging} className="flex items-center gap-2">
            <Activity className={`h-4 w-4 ${pinging ? "animate-pulse" : ""}`} />
            {pinging ? t("inspector.pinging") : t("inspector.pingServer")}
          </Button>
        </div>
      </div>
      ...
      {currentPing && (
        <div className="rounded-lg border p-4">
          <h4 className="text-sm font-semibold mb-3">{t("inspector.currentStatus")}</h4>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              {(() => {
                const StatusIcon = getStatusIcon(currentPing.success);
                return (
                  <StatusIcon className={`h-6 w-6 ${getStatusColor(currentPing.success)}`} />
                );
              })()}
              <div>
                <div className={`text-sm font-medium ${getStatusColor(currentPing.success)}`}>
                  {currentPing.success
                    ? t("inspector.serverResponsive")
                    : t("inspector.serverUnreachable")}
                </div>
                <div className="text-xs text-muted-foreground">
                  {t("inspector.responseTime")}: {formatDuration(currentPing.duration)}
                </div>
                <div className="text-xs text-muted-foreground">
                  {t("inspector.method")}: {currentPing.method}
                </div>
                {currentPing.error && (
                  <div className="text-xs text-red-600 mt-1">
                    {t("common.error")}: {currentPing.error}
                  </div>
                )}
              </div>
            </div>
      ...
      {pingHistory.length > 0 && (
        <div className="grid grid-cols-3 gap-4">
          <div className="rounded-lg border p-4 text-center">
            ...
            <div className="text-xs text-muted-foreground">{t("inspector.successRate")}</div>
          </div>
          <div className="rounded-lg border p-4 text-center">
            <div className="text-2xl font-bold text-blue-600">{getAverageResponseTime()}ms</div>
            <div className="text-xs text-muted-foreground">{t("inspector.avgResponse")}</div>
          </div>
          <div className="rounded-lg border p-4 text-center">
            <div className="text-2xl font-bold text-purple-600">{pingHistory.length}</div>
            <div className="text-xs text-muted-foreground">{t("inspector.totalPings")}</div>
          </div>
        </div>
      )}
      ...
      <div className="space-y-4">
        <h4 className="text-sm font-semibold">
          {t("inspector.pingHistory")} ({pingHistory.length})
        </h4>
        {pingHistory.length === 0 ? (
          <div className="rounded-lg border border-dashed p-8 text-center">
            <Clock className="h-8 w-8 text-muted-foreground mx-auto mb-2" />
            <p className="text-sm text-muted-foreground">{t("inspector.noPingHistory")}</p>
          </div>
        ) : (
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {pingHistory.slice(0, 10).map((ping) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    <StatusIcon className={`h-4 w-4 ${getStatusColor(ping.success)}`} />
                    <div>
                      <div className="text-sm">
                        <span className={`font-medium ${getStatusColor(ping.success)}`}>
                          {ping.success ? t("inspector.success") : t("inspector.failed")}
                        </span>
                      ...
                      {ping.error && <div className="text-xs text-red-600 mt-1">{ping.error}</div>}
                    </div>
                  </div>
                    ...
                    {Array.from({ length: 5 }).map((_, i) => (
                      <div
                        ...
                        className={`w-1 h-4 rounded ${
                          i <
                          (ping.success
                            ? Math.min(5, Math.max(1, 5 - Math.floor(ping.duration / 200)))
                            : 0)
                            ? ping.success
                              ? "bg-green-500"
                              : "bg-red-500"
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <h4 className="text-sm font-medium text-yellow-900 dark:text-yellow-100 mb-1">
              {t("inspector.aboutPing")}
            </h4>
            <p className="text-xs text-yellow-700 dark:text-yellow-300">
              {t("inspector.aboutPingDesc")}
            </p>
          </div>
        </div>
      </div>
