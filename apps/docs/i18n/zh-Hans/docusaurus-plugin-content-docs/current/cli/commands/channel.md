...
| 类型 | 描述 | 配置难度 |
|------|------|----------|
| `telegram` | Telegram Bot API | 简单（仅需 token）|
| `discord` | Discord Bot | 简单（bot token + intents）|
| `whatsapp` | WhatsApp 桥接 | 中等（扫描二维码）|
...
channels:
  my-telegram:
    type: telegram
    enabled: true
    token: "encrypted:xxx"  # 从 @BotFather 获取的 Bot token
  ...
  my-discord:
    type: discord
    enabled: true
    token: "encrypted:xxx"  # 从 Discord Developer Portal 获取的 Bot token
...
未配置渠道:
  使用 'viben channel types' 查看支持的渠道类型。
```
...
1. 在 Telegram 上找到 [@BotFather](https://t.me/BotFather)
2. 发送 `/newbot`
3. 按提示设置名称
4. 获取 Bot Token
