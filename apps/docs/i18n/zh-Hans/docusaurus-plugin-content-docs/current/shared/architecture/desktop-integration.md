...
Additional React hooks for agent orchestration and chat integration:
...
export function useAgents() {
  ...
  useEffect(() => {
    async function loadAgents() {
      ...
      const response = await fetch(\`\${gateway.baseUrl}/api/agent\`);
...
```typescript
// apps/desktop/src/hooks/use-chat-list.ts
export function useChatList(workspacePath?: string) {
  ...
  useEffect(() => {
    ...
    const eventSource = new EventSource(
      \`\${gateway.baseUrl}/api/chat-list?\${params}\`
    );
...
These hooks integrate with the Viben Gateway for real-time agent state and chat list management.
