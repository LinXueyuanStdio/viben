...
|-----------|------|----------|-------------|
| `searcher` | string | Yes | Source platform for download |
...
:::caution Paid Data Sources
Paid data sources (IEEE, Springer, ScienceDirect, Scopus) require institutional access or subscription to download PDFs. Even with an API key, PDF access depends on your subscription level.
:::
...
| Access denied | No subscription | Use free data sources or check subscription |
| Network error | Connection failed | Retry download |
...
- [browse_search](./browse-search) - Search for papers to download
