...
Skills can be published to and consumed from the Viben web marketplace. The marketplace provides:

- **Package Registry**: Search, browse, and install skills
- **Collections**: Curated lists of skill packages
- **Social Features**: Ratings, favorites, comments

See [Skill API](/backend/api/skill-api) and [Collections API](/backend/api/collections-api) for API details.
