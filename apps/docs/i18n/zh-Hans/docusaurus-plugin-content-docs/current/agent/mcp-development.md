...
MCP packages can be published to and consumed from the Viben web marketplace. The marketplace provides:

- **Package Registry**: Search, browse, and install MCP servers
- **Collections**: Curated lists of MCP packages
- **Social Features**: Ratings, favorites, comments

See [MCP API](/backend/api/mcp-api) and [Collections API](/backend/api/collections-api) for API details.
