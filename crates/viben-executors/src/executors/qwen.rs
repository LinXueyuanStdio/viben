...
impl QwenCode {
    /// Create a new Qwen Code executor
    pub fn new() -> Self {
        Self::default()
    }
}
...
impl StandardCodingAgentExecutor for QwenCode {
    ...
    async fn spawn(
        ...
        let mut cmd = Command::new("qwen-code");
    ...
    fn default_mcp_config_path(&self) -> Option<PathBuf> {
        dirs::config_dir().map(|config| config.join("qwen-code/config.json"))
    }

    fn get_availability_info(&self) -> AvailabilityInfo {
        if which::which("qwen-code").is_ok() {
