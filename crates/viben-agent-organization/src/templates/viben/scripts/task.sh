...
#
# Usage:
#   ./.viben/scripts/task.sh create "<title>" [--slug <name>] [--assignee <dev>] [--priority P0|P1|P2|P3]
#   ./.viben/scripts/task.sh init-context <dir> <type>   # Initialize jsonl files
#   ./.viben/scripts/task.sh add-context <dir> <file> <path> [reason] # Add jsonl entry
#   ./.viben/scripts/task.sh validate <dir>              # Validate jsonl files
#   ./.viben/scripts/task.sh list-context <dir>          # List jsonl entries
#   ./.viben/scripts/task.sh start <dir>                 # Set as current task
#   ./.viben/scripts/task.sh finish                      # Clear current task
#   ./.viben/scripts/task.sh set-branch <dir> <branch>   # Set git branch
#   ./.viben/scripts/task.sh set-scope <dir> <scope>     # Set scope for PR title
#   ./.viben/scripts/task.sh create-pr [dir] [--dry-run] # Create PR from task
#   ./.viben/scripts/task.sh archive <task-name>         # Archive completed task
#   ./.viben/scripts/task.sh list                        # List active tasks
#   ./.viben/scripts/task.sh list-archive [month]        # List archived tasks
#
...
# Claude: .claude/commands/viben/<name>.md
# Cursor: .cursor/commands/viben-<name>.md
get_command_path() {
  local name="$1"
  if [[ "$PLATFORM" == "cursor" ]]; then
    echo ".cursor/commands/viben-${name}.md"
  else
    echo ".claude/commands/viben/${name}.md"
  fi
}
...
cmd_set_branch() {
  ...
  echo "  ./.viben/scripts/multi-agent/start.sh $1"
}
...
Examples:
  ...
  $0 init-context .viben/tasks/01-21-add-login frontend --platform cursor
  $0 add-context <dir> implement .viben/spec/backend/auth.md "Auth guidelines"
