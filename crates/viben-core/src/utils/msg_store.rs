//! Message store for executor output aggregation and streaming
//!
//! Based on vibe-kanban's msg_store.rs implementation
...
use super::log_msg::{LogMsg, Patch};

/// Maximum history size in bytes (100 MB)
...
/// Message store for aggregating and streaming executor output
///
/// Features:
/// - Bounded history with automatic eviction
/// - Broadcast channel for live subscribers
/// - Combined history + live stream for late joiners
pub struct MsgStore {
    inner: RwLock<Inner>,
...
impl MsgStore {
    ...
    pub fn push(&self, msg: LogMsg) {
        ...
        // Evict old messages if we're over capacity
    ...
    /// Get a new receiver for live messages
    ...
    /// Get a stream of history then live messages
    ...
    /// Get SSE event stream (history + live)
    ...
    /// Spawn a forwarder task that pushes stream items to this store
