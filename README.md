...
graph TB
    ...
    subgraph Core["packages/core"]
        ...
        subgraph Services["服务层"]
            direction LR
            TaskSvc["TaskService<br/>状态机"]
            AgentSvc["AgentService<br/>会话管理"]
            CronSvc["CronService<br/>定时任务"]
        end

        subgraph Modules["功能模块"]
            direction LR
            Exec["Executors<br/>9种执行器"]
            MCP["MCP Client<br/>协议支持"]
            Chan["Channels<br/>通知渠道"]
        end
    end

    subgraph Storage["存储层 ~/.viben/"]
        direction LR
...
> **设计原则**: `packages/core` 是所有应用的唯一边界 | 文件原生配置 (YAML/JSON) | 统一 REST/WebSocket API | 多 Provider 支持
